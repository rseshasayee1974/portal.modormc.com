    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                toast.add({ severity: 'success', summary: 'Deleted', detail: 'Currency removed successfully', life: 1500 });
            } catch (error) {
                toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to delete currency', life: 1500 });
            }
        }
    });
};
    ...
    <AppLayout title="Currencies">
        ...
        <div class="p-6">
            <div class="card bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 rounded-lg">
                <DataTable :value="store.currencies" stripedRows class="p-datatable-sm text-sm" :paginator="true" :rows="30">
                    <template #header>
