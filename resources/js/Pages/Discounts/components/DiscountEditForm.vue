...
const props = defineProps({
    discount: { type: Object as PropType<Discount>, required: true },
...
const values = () => ({
    ...
    value: props.discount.value !== null && props.discount.value !== undefined ? Number(props.discount.value) : (null as number | null),
    amount: props.discount.amount !== null && props.discount.amount !== undefined ? Number(props.discount.amount) : (null as number | null),
    ...
    date: props.discount.date ? props.discount.date.slice(0, 10) : '',
...
const partnerOptions = computed(() => {
    ...
    return props.partners.filter(p => {
        // Retain currently saved partner during edit
        if (p.id === props.discount.partner_id) {
            return true;
        }
...
const journalOptions = computed(() => {
    ...
    const filtered = props.journals.filter(j => {
        ...
        if (j.applied_discount_id && Number(j.applied_discount_id) !== Number(props.discount.id)) {
            return false;
        }
...
watch(
    () => form.partner_id,
    (newPartnerId) => {
        if (newPartnerId && form.journal_id && form.journal_id !== props.discount.journal_id) {
    ...
    const options = {
        ...
        onSuccess: () => {
            ...
            Swal.fire({
                ...
                position: 'top-end',
                icon: 'success',
                title: `Discount #${props.discount.id} updated successfully`,
                timer: 1600,
        ...
        onError: (errors: any) => {
            Swal.fire({
                icon: 'error',
                title: 'Update Failed',
                text: Object.values(errors).flat()[0] as string || 'Please check form validation errors.',
            });
        }
    };
    ...
    <form @submit.prevent="submit"
        ...
        <div class="flex items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <Tag :value="discount.primary_type"
                            :severity="discount.primary_type === 'Sales' ? 'info' : 'warn'"
                            class="text-[10px] font-bold uppercase tracking-wider" />
                    </div>
                    <p class="text-xs text-slate-500 mt-0.5">
                        Modify recorded parameters, ledger account, or monetary value for discount voucher #{{
                            discount.id }}.
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-mono text-slate-400 bg-slate-50 px-2 py-1 rounded border border-slate-200">
                    {{ discount.date?.slice(0, 10) }}
                </span>
            </div>
        </div>

        <!-- Form Fields Grid -->
        <fieldset :disabled="readonly || form.processing" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <!-- Transaction Type -->
            ...
            <BaseSelect v-model="form.partner_id"
                ...
                :error="form.errors.partner_id" :disabled="readonly || form.processing"
            ...
            <BaseSelect v-model="form.journal_id"
                ...
                :placeholder="form.partner_id ? (form.primary_type === 'Sales' ? 'Select invoice for this customer' : 'Select bill for this supplier') : (form.primary_type === 'Sales' ? 'Select invoice or receipt voucher' : 'Select bill or payment voucher')"
                :error="form.errors.journal_id" :disabled="readonly || form.processing"
            ...
            <div v-if="selectedJournal && selectedVoucherAmount > 0"
                class="col-span-1 sm:col-span-2 lg:col-span-4 rounded-xl border border-indigo-100 bg-gradient-to-r from-indigo-50/90 via-sky-50/40 to-white p-3.5 shadow-xs flex flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <div
            ...
            <BaseSelect v-model="form.account_id" label="Discount Account (Ledger)" :options="accounts"
                ...
                :error="form.errors.account_id" :disabled="readonly || form.processing" hint="Linked ledger line" />
            ...
            <BaseInputNumber v-model="form.amount" label="Net Discount Amount (₹)" prefix="₹ " :min="0.01"
                ...
                :hint="form.value_type === 'percent' ? 'Enter monetary discount amount corresponding to percentage.' : 'Automatically matched with fixed value.'"
                :error="form.errors.amount" required />

            <!-- Discount Reference Number -->
            <BaseInput v-model="form.reference_number" label="Reference Number" placeholder="Auto-generated if empty"
                hint="e.g. SDISC/2627/00001 or custom ref" :error="form.errors.reference_number"
                :disabled="readonly || form.processing" />

            <!-- Move Reference taken automatically from selected voucher -->
            <!-- <BaseInputNumber
                v-model="form.move_id"
                label="Move Reference (Optional)"
                placeholder="e.g. 1001"
                :min="1"
                :max="2147483647"
                :maxFractionDigits="0"
                :useGrouping="false"
                :error="form.errors.move_id"
                :disabled="readonly || form.processing"
            /> -->
