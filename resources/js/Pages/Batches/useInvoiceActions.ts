    ...
    const generateEwayBillDirect = async (batchOrInvoice: any, callback?: () => void) => {
        ...
        Swal.fire({
                ...
                <div class="text-left space-y-3.5 text-slate-700 dark:text-slate-200">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input id="swal-ewb-distance" type="number" min="1" step="1" placeholder="Automatic from PIN codes" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm dark:bg-slate-700 dark:border-slate-600 dark:text-white" />
                            <p class="mt-1 text-[10px] text-slate-500">Optional. Leave blank to use the distance calculated from PIN ${escapeHtml(delivery.from_zipcode)} to ${escapeHtml(delivery.to_zipcode)}.</p>
                        </div>
                    </div>
            ...
            preConfirm: () => {
                ...
                if (distance && (!/^\d+$/.test(distance) || Number(distance) < 1)) {
                    Swal.showValidationMessage('Distance must be a whole number of kilometres');
                    return false;
                }
                return { 
                    vehNo, 
            ...
            if (result.isConfirmed && result.value) {
                ...
                try {
                    ...
                    const res = await axios.post(postUrl, {
                        ...
                        distance_km: result.value.distance,
