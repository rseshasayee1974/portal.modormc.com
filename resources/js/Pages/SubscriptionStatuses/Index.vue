...
import { useSubscriptionStatusStore } from '@/Pages/SubscriptionStatuses/useSubscriptionStatusStore';
...
const store = useSubscriptionStatusStore();
...
onMounted(() => {
    store.setSubscriptionStatuses(props.subscriptionStatuses);
});

const deleteSubscriptionStatus = (id: number) => {
    Swal.fire({
        title: 'Are you sure?',
...
const openEditModal = (subscriptionStatus: SubscriptionStatus) => {
    modalMode.value = 'edit';
    editingId.value = subscriptionStatus.id;
    modalForm.value.status_name = subscriptionStatus.status_name;
    modalForm.value.errors.status_name = '';
    showModal.value = true;
};

const openViewModal = (subscriptionStatus: SubscriptionStatus) => {
    openEditModal(subscriptionStatus);
...
const submitModal = async () => {
    ...
    try {
        ...
        if (modalMode.value === 'create') {
            const response = await axios.post(route('subscriptionstatuses.store'), payload);
            store.addSubscriptionStatus(response.data.subscriptionStatus);
            toast.add({ severity: 'success', summary: 'Success', detail: 'Subscription status created' });
        } else {
            const response = await axios.put(route('subscriptionstatuses.update', editingId.value), payload);
            store.updateSubscriptionStatus(response.data.subscriptionStatus);
            toast.add({ severity: 'success', summary: 'Success', detail: 'Subscription status updated' });
        }
        closeModal();
    } catch (error: any) {
...
<template>
    <AppLayout title="Subscription Statuses">
        <template #header>
            <ModuleSubTopNav />
        </template>
        ...
        <div class="p-6">
            <div class="card bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 rounded-lg">
                <DataTable :value="store.subscriptionStatuses" stripedRows class="p-datatable-sm text-sm" :paginator="true" :rows="10">
                    <template #header>
                        <div class="flex items-center justify-between">
                            <span class="text-xl font-semibold uppercase tracking-tight">Recurring Service States</span>
                    ...
                    <Column header="Actions" class="text-right" style="width: 120px">
                        <template #body="slotProps">
                            <div class="flex justify-end gap-2">
                                ...
                                <Button icon="pi pi-trash" text rounded  @click="deleteSubscriptionStatus(slotProps.data.id)" severity="danger" />
                            </div>
                        </template>
                    </Column>
                </DataTable>
        ...
        <Dialog v-model:visible="showModal" modal :header="modalMode.toUpperCase() + ' SUBSCRIPTION STATUS'" :style="{ width: '450px' }">
