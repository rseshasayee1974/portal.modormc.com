    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-5 items-start">
        ...
        <div class="lg:col-span-1">
            <div v-if="kind === 'patron'">
                <BaseSelect :modelValue="form.patron_id" @update:modelValue="selectTarget" :options="patrons"
                    optionLabel="label" optionValue="value" label="Customer or Vendor" :required="true"
                    :disabled="busy || !!form.record_id" placeholder="Select customer or vendor..."
                    :filterFields="['label', 'legal_name', 'code']" :filter="true" :showClear="true" />
                <!-- {{ selectedPatronDetail }} -->
                <div v-if="selectedPatronDetail"
                    class="mt-1 flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                    <span>Mapped Ledger:</span>
                    <span class="font-semibold text-indigo-600 dark:text-indigo-400">
                        {{ form.side === 'Dr' ?
                            (ledgerName(selectedPatronDetail.debit_ledger_id) || 'Default Debit Ledger')
                            : (ledgerName(selectedPatronDetail.credit_ledger_id) || 'Default Credit Ledger') }}
                    </span>
                </div>
            </div>
            <div v-else>
                <BaseSelect :modelValue="form.account_id" @update:modelValue="value => selectTarget(value)"
                    :options="ledgers" optionLabel="label" optionValue="value" label="General Ledger Account"
                    :required="true" :disabled="busy || !!form.record_id" placeholder="Select general ledger account..."
                    :filterFields="['label', 'title', 'code']" :filter="true" :showClear="true" />
                <div v-if="selectedLedgerDetail" class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                    <span>Account Code: </span>
                    <strong class="font-mono text-slate-700 dark:text-slate-300">{{ selectedLedgerDetail.code || 'N/A'
                        }}</strong>
                </div>
            </div>
        </div>
        ...
        <div>
            <BaseDatePicker :model-value="form.cutover_date" label="Cutover Date" dateFormat="yy-mm-dd" :required="true"
                :disabled="true" placeholder="YYYY-MM-DD" />
        </div>

        <!-- 3. Balance Side (BaseSelect + Quick Buttons) -->
        <div>
            <BaseSelect v-model="form.side" :options="sideOptions" optionLabel="label" optionValue="value"
                label="Balance Side" :required="true" :disabled="busy" />
            <div class="grid grid-cols-2 gap-2 mt-2">
                <button type="button" @click="form.side = 'Dr'" :disabled="busy" :class="[
                ...
                <button type="button" @click="form.side = 'Cr'" :disabled="busy" :class="[
                    'py-1.5 px-2.5 rounded-lg border text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer',
                    form.side === 'Cr'
