    ...
    <div v-if="reportData" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div class="bg-white p-4 rounded border border-slate-200 shadow-sm">
                <span class="text-[10px] font-black uppercase tracking-wider text-slate-400">Total Trips</span>
                <p class="text-xl font-black text-slate-800 mt-1">{{ reportData.totals?.trips_count || 0 }}</p>
            </div>
            <div class="bg-white p-4 rounded border border-slate-200 shadow-sm">
                <span class="text-[10px] font-black uppercase tracking-wider text-slate-400">Total Batch Size</span>
                <p class="text-xl font-black text-indigo-700 mt-1">{{ formatQuantity(reportData.totals?.batch_size) }} m³</p>
            </div>
            <div class="bg-white p-4 rounded border border-slate-200 shadow-sm">
                <span class="text-[10px] font-black uppercase tracking-wider text-slate-400">Total Delivered Qty</span>
                <p class="text-xl font-black text-blue-700 mt-1">{{ formatQuantity(reportData.totals?.quantity) }} m³</p>
            </div>
        </div>

        <!-- Driver Consolidated Table -->
        <div class="bg-white rounded border border-slate-200 shadow-sm">
            <div class="px-5 py-3 border-b border-slate-100 bg-slate-50/60 flex justify-between items-center">
                <div>
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700">Driver Trip & Dispatch Performance Summary</h3>
                    <p class="text-[10px] text-slate-400">Driver wise trips, delivered volume, and batch sizes</p>
                </div>
                ...
                <table class="w-full text-left border-collapse min-w-[750px]">
                    <thead>
                        <tr class="text-[10px] font-bold uppercase tracking-wider text-slate-600 border-b border-slate-200 bg-[#f8fafc]">
                            ...
                            <th class="py-3 px-3" width="35%">Driver Name</th>
                            <th class="py-3 px-3 text-center" width="15%">Code</th>
                            <th class="py-3 px-3 text-center" width="15%">Trips</th>
                            <th class="py-3 px-3 text-right" width="15%">Batch Size (m³)</th>
                            <th class="py-3 px-3 text-right" width="15%">Delivered Qty (m³)</th>
                        </tr>
                    </thead>
                    <tbody class="text-[11px] font-semibold text-slate-700 divide-y divide-slate-100">
                        <tr v-for="(row, idx) in paginatedList" :key="idx" class="hover:bg-slate-50 transition-all">
                            ...
                            <td class="py-2.5 px-3 font-bold text-slate-800">
                                {{ row.driver_name }}
                                <span v-if="row.driver_mobile" class="text-[10px] text-slate-400 block font-normal">{{ row.driver_mobile }}</span>
                            </td>
                            <td class="py-2.5 px-3 text-center text-slate-500">{{ row.driver_code || '-' }}</td>
                        ...
                        <tr v-if="!consolidatedList.length">
                            <td colspan="6" class="py-8 text-center text-slate-400">No driver records found for the selected period</td>
                        </tr>
        ...
        <div v-if="reportData.driver_vehicle_summary?.length" class="bg-white rounded border border-slate-200 shadow-sm">
            <div class="px-5 py-3 border-b border-slate-100 bg-slate-50/60 flex justify-between items-center">
                <div>
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700">Driver & Vehicle Trips Breakdown</h3>
                    <p class="text-[10px] text-slate-400">Vehicle / Truck transit mixer wise trip and volume distribution per driver</p>
                </div>
                <span class="text-xs font-bold text-slate-500">
                    {{ reportData.driver_vehicle_summary.length }} Vehicle Links
                </span>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse min-w-[750px]">
                    <thead>
                        <tr class="text-[10px] font-bold uppercase tracking-wider text-slate-600 border-b border-slate-200 bg-[#f8fafc]">
                            <th class="py-2.5 px-3 text-center" width="6%">#</th>
                            <th class="py-2.5 px-3" width="38%">Driver Name</th>
                            <th class="py-2.5 px-3" width="26%">Vehicle / Truck Reg</th>
                            <th class="py-2.5 px-3 text-center" width="15%">Trips</th>
                            <th class="py-2.5 px-3 text-right" width="15%">Delivered Qty (m³)</th>
                        </tr>
                    </thead>
                    <tbody class="text-[11px] font-semibold text-slate-700 divide-y divide-slate-100">
                        <tr v-for="(row, idx) in reportData.driver_vehicle_summary" :key="idx" class="hover:bg-slate-50 transition-all">
                            <td class="py-2 px-3 text-center text-slate-400">{{ idx + 1 }}</td>
                            <td class="py-2 px-3 font-bold text-slate-800">{{ row.driver_name }}</td>
                            <td class="py-2 px-3 font-bold text-slate-700">{{ row.truck_no }}</td>
