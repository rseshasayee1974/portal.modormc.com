...
<template>
    <div v-if="reportData" class="space-y-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 2xl:grid-cols-7 gap-3">
            <div
                class="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-sm min-w-0 flex flex-col justify-between hover:border-slate-300 transition-colors">
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 truncate">Total Trips</span>
                <p class="text-sm sm:text-base font-black text-slate-800 mt-1 truncate tracking-tight"
                    :title="`${reportData.total_trips || 0}`">{{ reportData.total_trips || 0 }}</p>
            </div>
            <div
                class="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-sm min-w-0 flex flex-col justify-between hover:border-slate-300 transition-colors">
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 truncate">Batch Size</span>
                <p class="text-sm sm:text-base font-black text-indigo-700 mt-1 truncate tracking-tight"
                    :title="`${formatQuantity(reportData.total_batch_size)} m³`">
                    {{ formatQuantity(reportData.total_batch_size) }} <span
                        class="text-xs font-semibold text-indigo-400">m³</span>
                </p>
            </div>
            <div
                class="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-sm min-w-0 flex flex-col justify-between hover:border-slate-300 transition-colors">
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 truncate">Delivered
                    Qty</span>
                <p class="text-sm sm:text-base font-black text-blue-700 mt-1 truncate tracking-tight"
                    :title="`${formatQuantity(reportData.total_quantity)} m³`">
                    {{ formatQuantity(reportData.total_quantity) }} <span
                        class="text-xs font-semibold text-blue-400">m³</span>
                </p>
            </div>
            <div
                class="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-sm min-w-0 flex flex-col justify-between hover:border-slate-300 transition-colors">
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 truncate">Empty Wt</span>
                <p class="text-sm sm:text-base font-bold text-slate-600 mt-1 truncate tracking-tight"
                    :title="`${formatQuantity(reportData.total_truck_empty)} T`">
                    {{ formatQuantity(reportData.total_truck_empty) }} <span
                        class="text-xs font-medium text-slate-400">T</span>
                </p>
            </div>
            <div
                class="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-sm min-w-0 flex flex-col justify-between hover:border-slate-300 transition-colors">
                ...
                <p class="text-sm sm:text-base font-bold text-slate-600 mt-1 truncate tracking-tight"
                    :title="`${formatQuantity(reportData.total_loaded_weight)} T`">
                    {{ formatQuantity(reportData.total_loaded_weight) }} <span
                        class="text-xs font-medium text-slate-400">T</span>
                </p>
            </div>
            <div
                ...
                <p class="text-sm sm:text-base font-black text-emerald-700 mt-1 truncate tracking-tight"
                    :title="`${formatQuantity(reportData.total_net_weight)} T`">
                    {{ formatQuantity(reportData.total_net_weight) }} <span
                        class="text-xs font-semibold text-emerald-400">T</span>
                </p>
            </div>
            <div
                class="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-sm min-w-0 flex flex-col justify-between hover:border-slate-300 transition-colors">
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 truncate">Total
                    Revenue</span>
                <p class="text-sm sm:text-base font-black text-[#1d2d3e] mt-1 truncate tracking-tight"
                    :title="formatCurrency(reportData.total_amount)">
        ...
        <div class="flex items-center gap-2 border-b border-slate-200 bg-white px-5 pt-3 rounded-t-xl shadow-2xs">
            <button type="button" @click="activeTab = 'summary'" :class="[
                ...
                <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold"
                    :class="activeTab === 'summary' ? 'bg-blue-100 text-[#0064d2]' : 'bg-slate-100 text-slate-500'">
                    {{ totalCount }} Customers
                </span>
            </button>
            <button v-if="allTrips.length" type="button" @click="activeTab = 'trips'" :class="[
                'pb-3 px-3 text-xs font-bold transition-all border-b-2 flex items-center gap-2 cursor-pointer',
                activeTab === 'trips'
                    ? 'border-[#0064d2] text-[#0064d2]'
                    : 'border-transparent text-slate-500 hover:text-slate-800'
                ...
                <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold"
                    :class="activeTab === 'trips' ? 'bg-blue-100 text-[#0064d2]' : 'bg-slate-100 text-slate-500'">
        ...
        <div v-show="activeTab === 'summary'"
            class="bg-white rounded-b-xl border border-t-0 border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-3 border-b border-slate-100 bg-slate-50/60 flex justify-between items-center">
                <div>
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700">Customer Consolidated Summary
                    </h3>
                    <p class="text-[10px] text-slate-400">Customer / party wise dispatches consolidated with trip count,
                        batch size, and weights</p>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-[11px] font-semibold text-slate-700 divide-y divide-slate-100">
                        <tr v-for="(row, idx) in paginatedRows" :key="idx" class="hover:bg-slate-50 transition-all">
                            <td class="py-2.5 px-3 text-center text-slate-400 font-bold">{{ (sanitizedPage - 1) *
                                perPage + idx + 1 }}</td>
                            ...
                            <td class="py-2.5 px-3 text-right font-bold text-slate-900 whitespace-nowrap">{{
                                formatQuantity(row.quantity) }}</td>
                            <td
                                class="py-2.5 px-3 text-right font-bold text-[#1d2d3e] bg-slate-50/55 whitespace-nowrap">
                                {{ formatCurrency(row.amount_total) }}</td>
                            <td class="py-2.5 px-3 text-center">
                                <button type="button"
                                    @click="emit('view-statement', row.customer_id || row.patron_id || row.id)"
                                    :disabled="!(row.customer_id || row.patron_id || row.id)"
                        ...
                        <tr v-if="!paginatedRows?.length">
                            <td colspan="6" class="py-8 text-center text-slate-400">No customer dispatches found for
                                selected period</td>
                        </tr>
                    </tbody>
                    <tfoot
                        class="sticky bottom-0 bg-[#f2f4f7] font-bold border-t-2 border-slate-300 text-xs shadow-2xs z-10">
                        <tr>
                            <td colspan="2" class="py-3 px-3 text-center text-[#1d2d3e] uppercase">Total Customer Volume
                            </td>
                            <td class="py-3 px-3 text-center text-[#1d2d3e] font-black">{{ reportData.total_trips }}
                            </td>
                            <td class="py-3 px-3 text-right text-[#1d2d3e] font-black whitespace-nowrap">{{
                                formatQuantity(reportData.total_quantity) }}</td>
                            <td class="py-3 px-3 text-right text-[#1d2d3e] font-black whitespace-nowrap">{{
                                formatCurrency(reportData.total_amount) }}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
            ...
            <div v-if="totalCount > perPage || totalPages > 1"
                class="px-4 py-3 bg-slate-50 border-t border-slate-200 rounded-b flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                <div class="flex items-center gap-3 text-slate-500 font-medium">
                    <span>
                        Showing <strong class="text-slate-800">{{ startIndex }}</strong> to <strong
                            class="text-slate-800">{{ endIndex }}</strong> of <strong class="text-slate-800">{{
                            totalCount }}</strong> customers
                    </span>
                ...
                <div class="flex items-center gap-1">
                    <button type="button" @click="goToPage(1)" :disabled="sanitizedPage <= 1"
                    ...
                    <button type="button" @click="goToPage(sanitizedPage - 1)" :disabled="sanitizedPage <= 1"
                    ...
                    <button type="button" @click="goToPage(sanitizedPage + 1)" :disabled="sanitizedPage >= totalPages"
                        class="px-2.5 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-[11px] cursor-pointer">
                        Next ›
                    </button>
                    <button type="button" @click="goToPage(totalPages)" :disabled="sanitizedPage >= totalPages"
                        class="px-2 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-[11px] cursor-pointer"
                        title="Last Page">
                        Last »
                    </button>
        ...
        <div v-if="allTrips.length" v-show="activeTab === 'trips'"
            class="bg-white rounded-b-xl border border-t-0 border-slate-200 shadow-sm overflow-hidden">
            <div
                class="px-5 py-4 border-b border-slate-100 bg-slate-50/70 flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div>
                ...
                <table class="w-full text-left border-collapse min-w-[950px]">
                    ...
                    <tbody class="text-[11px] font-semibold text-slate-700 divide-y divide-slate-100">
                        <tr v-for="(trip, tIdx) in paginatedTrips" :key="tIdx" class="hover:bg-slate-50 transition-all">
                            <td class="py-2.5 px-3 text-center text-slate-400 font-bold">{{ (sanitizedTripPage - 1) *
                                tripPerPage + tIdx + 1 }}</td>
                            ...
                            <td class="py-2.5 px-3 text-center font-bold text-emerald-700">{{ trip.concrete_grade }}
                            </td>
                            <td class="py-2.5 px-3 text-right font-bold text-slate-900">{{
                                formatQuantity(trip.delivered_qty) }}
                            </td>
                            <td class="py-2.5 px-3 text-right text-slate-600">{{ formatQuantity(trip.empty_weight) }}
                            </td>
                            <td class="py-2.5 px-3 text-right text-slate-600">{{ formatQuantity(trip.loaded_weight) }}
                            </td>
                            <td class="py-2.5 px-3 text-right font-semibold text-slate-800">{{
                                formatQuantity(trip.net_weight)
                                }}</td>
                            <td
                                class="py-2.5 px-3 text-right font-bold text-[#1d2d3e] bg-slate-50/55 whitespace-nowrap">
                                {{
                                formatCurrency(trip.amount_total) }}</td>
                        </tr>
                        <tr v-if="!paginatedTrips.length">
                            <td colspan="12" class="py-8 text-center text-slate-400">No batching trips match the search
                                criteria
                            </td>
                        </tr>
                    </tbody>
                    <tfoot
                        class="sticky bottom-0 bg-[#f2f4f7] font-bold border-t-2 border-slate-300 text-xs shadow-2xs z-10">
                        <tr>
                            <td colspan="7" class="py-3 px-3 text-center text-[#1d2d3e] uppercase">Total Verified Batch
                                Trips
                                ({{ tripTotalCount }} Trips)</td>
                            <td class="py-3 px-3 text-right text-[#1d2d3e] font-black whitespace-nowrap">{{
                                formatQuantity(tripTotals.delivered_qty) }}</td>
                            <td class="py-3 px-3 text-right text-[#1d2d3e] font-black whitespace-nowrap">{{
                                formatQuantity(tripTotals.empty_weight) }}</td>
                            <td class="py-3 px-3 text-right text-[#1d2d3e] font-black whitespace-nowrap">{{
                                formatQuantity(tripTotals.loaded_weight) }}</td>
                            <td class="py-3 px-3 text-right text-[#1d2d3e] font-black whitespace-nowrap">{{
                                formatQuantity(tripTotals.net_weight) }}</td>
                            <td class="py-3 px-3 text-right text-[#1d2d3e] font-black whitespace-nowrap">{{
                                formatCurrency(tripTotals.amount_total) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div v-if="tripTotalCount > tripPerPage || tripTotalPages > 1"
                class="px-4 py-3 bg-slate-50 border-t border-slate-200 rounded-b flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                <div class="flex items-center gap-3 text-slate-500 font-medium">
                    <span>
                        Showing <strong class="text-slate-800">{{ tripStartIndex }}</strong> to <strong
                            class="text-slate-800">{{ tripEndIndex }}</strong> of <strong class="text-slate-800">{{
                            tripTotalCount }}</strong> batch trips
                    </span>
                    <div class="flex items-center gap-1.5 ml-2">
                        <span class="text-[11px] text-slate-400">Rows:</span>
                        <select v-model.number="tripPerPage" @change="tripCurrentPage = 1"
                            class="border border-slate-200 rounded px-2 py-0.5 text-xs text-slate-700 bg-white font-semibold focus:outline-none focus:ring-1 focus:ring-[#0064d2]">
                            <option :value="15">15</option>
                            <option :value="30">30</option>
                            <option :value="50">50</option>
                ...
                <div class="flex items-center gap-1">
                    <button type="button" @click="goToTripPage(1)" :disabled="sanitizedTripPage <= 1"
                        class="px-2 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-[11px] cursor-pointer"
                        title="First Page">
                        « First
                    </button>
                    <button type="button" @click="goToTripPage(sanitizedTripPage - 1)"
                    ...
                    <template v-for="(p, pIdx) in visibleTripPages" :key="pIdx">
                        <span v-if="p === '...'" class="px-1 text-slate-400 font-semibold">...</span>
                        <button v-else type="button" @click="goToTripPage(p)" :class="[
                            'px-2.5 py-1 rounded text-[11px] font-bold transition-all cursor-pointer',
                            sanitizedTripPage === p
                                ? 'bg-[#0064d2] text-white shadow-xs'
                                : 'border border-slate-200 text-slate-600 hover:bg-slate-100'
                    ...
                    <button type="button" @click="goToTripPage(tripTotalPages)"
                        :disabled="sanitizedTripPage >= tripTotalPages"
                        class="px-2 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-[11px] cursor-pointer"
                        title="Last Page">
                        Last »
