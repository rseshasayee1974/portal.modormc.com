    ...
    <div v-if="reportData" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white p-4 rounded-xl border border-rose-200 shadow-sm">
                ...
                <p class="text-2xl font-black text-rose-700 mt-1">{{ reportData.total_deleted ?? transactions.length }}</p>
            ...
            <div class="bg-white p-4 rounded-xl border border-blue-200 shadow-sm">
                ...
                <p class="text-2xl font-black text-blue-700 mt-1">{{ totalRows }}</p>
            ...
            <div class="bg-white p-4 rounded-xl border border-emerald-200 shadow-sm">
                ...
                <p class="text-2xl font-black text-emerald-700 mt-1">{{ formatCurrency(filteredTotalAmount) }}</p>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">
                        Log of deleted invoices, bills, payments, receipts, batches, dispatches, expenses, e-way bills, journals & discounts.
                    </p>
                </div>
                <div class="flex items-center gap-3">
                    <div class="relative">
                        <input
                            v-model="searchQuery"
                            type="text"
                            placeholder="Search ref #, customer, user..."
                ...
                <table class="w-full text-left text-xs border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        >
                            ...
                            <td class="py-3 px-4">
                                <span :class="['px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider inline-block', getTypeBadgeClass(row.entity_type)]">
                                    {{ row.entity_type }}
                                </span>
                            </td>
                            ...
                            <td class="py-3 px-4 whitespace-nowrap">
                                <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 text-[11px] font-medium border border-slate-200">
                        ...
                        <tr v-if="paginatedList.length === 0">
                            <td colspan="9" class="py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center space-y-2">
                                    ...
                                    <p class="text-sm font-semibold">No deleted records found matching current criteria.</p>
                                    <p class="text-xs text-slate-400">Try adjusting the customer filter or date range.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
                    <tfoot v-if="filteredList.length > 0" class="bg-slate-50 font-bold border-t-2 border-slate-200 text-slate-700">
                        <tr>
                            ...
                            <td class="py-3 px-4 text-right text-sm font-black text-rose-700">
                                {{ Number(filteredTotalAmount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
                            </td>
                            <td></td>
                        </tr>
                    </tfoot>
            ...
            <!-- Pagination Footer -->
            <div class="px-5 py-3 border-t border-slate-100 bg-slate-50/40 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
                <div class="flex items-center gap-2">
                    ...
                    <span>
                        Showing {{ totalRows > 0 ? (sanitizedPage - 1) * perPage + 1 : 0 }} - {{ Math.min(sanitizedPage * perPage, totalRows) }} of {{ totalRows }} records
                    </span>
                </div>
