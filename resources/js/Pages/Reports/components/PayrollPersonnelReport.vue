...
<template>
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <!-- Total Employees -->
            <div class="border border-slate-200 rounded-lg p-4 bg-white shadow-2xs flex justify-between items-center transition-all hover:border-slate-300">
                <div>
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Total Personnel</span>
            ...
            <div class="border border-slate-200 rounded-lg p-4 bg-white shadow-2xs flex justify-between items-center transition-all hover:border-slate-300">
                <div>
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Payslips Status</span>
            ...
            <div class="border border-slate-200 rounded-lg p-4 bg-white shadow-2xs flex justify-between items-center transition-all hover:border-slate-300">
                <div>
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Gross Earnings</span>
                    <span class="text-lg font-black text-slate-800 mt-1 block">{{ formatCurrency(summary.totalEarnings) }}</span>
                </div>
            ...
            <div class="border border-slate-200 rounded-lg p-4 bg-white shadow-2xs flex justify-between items-center transition-all hover:border-slate-300">
                <div>
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Total Deductions</span>
                    <span class="text-lg font-black text-rose-700 mt-1 block">{{ formatCurrency(summary.totalDeductions) }}</span>
                </div>
            ...
            <!-- Net Salary Outflow -->
            <div class="border border-slate-200 rounded-lg p-4 bg-[#f2f7fc] border-blue-200 shadow-2xs flex justify-between items-center transition-all hover:border-blue-300">
                <div>
                    <span class="text-[10px] font-bold text-[#0064d2] uppercase tracking-wider block">Net Salary Payout</span>
                    <span class="text-xl font-black text-[#0064d2] mt-1 block">{{ formatCurrency(summary.totalNetSalary) }}</span>
                </div>
                <div class="w-10 h-10 rounded-full bg-white border border-blue-200 flex items-center justify-center text-[#0064d2]">
                    <BanknotesIcon class="w-5 h-5" />
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white p-3.5 border border-slate-200 rounded-t-lg border-b-0">
            <div class="flex items-center gap-3">
                <span class="text-xs font-bold text-[#1d2d3e] uppercase tracking-wider">
                    Staff Payslip & Personnel Register
                </span>
                <span class="text-[11px] px-2.5 py-0.5 rounded-full font-bold bg-slate-100 text-slate-600 border border-slate-200">
        ...
        <div class="overflow-x-auto border border-slate-200 rounded-b-lg bg-white shadow-2xs">
            <table class="w-full text-left border-collapse min-w-[1050px]">
                ...
                <tbody class="text-[11px] font-semibold text-slate-700 divide-y divide-slate-100">
                    <tr v-for="(row, idx) in filteredTransactions" :key="idx" class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="py-3 px-3.5 text-center">
                            <div v-if="row.has_payslip" class="inline-flex items-center gap-1.5 text-[10px]">
                                <span class="text-emerald-700 font-bold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100" title="Present Days">
                                    P: {{ row.present_days }}
                                </span>
                                <span v-if="row.paid_leave_days > 0" class="text-blue-700 font-bold bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100" title="Paid Leave Days">
                                    L: {{ row.paid_leave_days }}
                                </span>
                                <span v-if="row.absent_days > 0" class="text-rose-700 font-bold bg-rose-50 px-1.5 py-0.5 rounded border border-rose-100" title="Absent Days">
                                    A: {{ row.absent_days }}
                                </span>
                            </div>
                        ...
                        <td class="py-3 px-3.5 text-right font-mono text-rose-600">
                            {{ row.has_payslip && row.total_deductions > 0 ? ('- ' + formatCurrency(row.total_deductions)) : (row.has_payslip ? '₹0.00' : '—') }}
                        </td>
                        ...
                        <td class="py-3 px-3.5 text-center">
                            <div v-if="row.has_payslip" class="flex items-center justify-center gap-1.5">
                                ...
                                <button 
                                    type="button" 
                                    @click="viewPayslip(row.payslip_id)"
                                ...
                                <button 
                                    type="button" 
                                    @click="downloadPayslip(row.payslip_id)"
                    ...
                    <tr v-if="filteredTransactions.length === 0">
                        <td colspan="11" class="py-12 text-center text-slate-400">
                            ...
                            <p class="text-xs font-semibold text-slate-500">No personnel records found for the selected query.</p>
                            <p class="text-[11px] text-slate-400 mt-1">Try selecting a different month, date range, or clear employee filters.</p>
                        </td>
                    </tr>
                </tbody>
            </table>
