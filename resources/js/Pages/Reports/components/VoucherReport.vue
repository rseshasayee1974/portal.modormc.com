    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="bg-white p-4 rounded border border-slate-200 shadow-sm flex items-center justify-between">
                ...
                <div
                    class="w-10 h-10 rounded bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 font-bold text-sm">
                    #
                </div>
            </div>
            ...
            <div class="bg-white p-4 rounded border border-slate-200 shadow-sm flex items-center justify-between">
                ...
                <div class="w-10 h-10 rounded border flex items-center justify-center font-bold text-sm"
            ...
            <div
                class="bg-white p-4 rounded border border-slate-200 shadow-sm flex items-center justify-between sm:col-span-2">
                <div>
                ...
                <span class="px-2.5 py-1 rounded text-[10px] font-black uppercase tracking-wider border"
            ...
            <table class="w-full text-left border-collapse min-w-[850px]">
                <thead>
                    <tr
                        ...
                        <th class="py-3 px-4" width="22%">{{ isPayment ? 'Paid To (Party)' : 'Received From (Party)' }}
                        </th>
                        <th class="py-3 px-4" width="18%">
                            {{ isPayment ? 'Paid From (Account)' : 'Received Into (Account)' }}</th>
                ...
                <tbody class="text-[11px] font-semibold text-slate-700">
                    <tr v-for="(trx, idx) in transactions" :key="trx.id || idx"
                        ...
                        <td class="py-3 px-4 text-center text-slate-600 whitespace-nowrap">{{ formatDate(trx.date) }}
                        </td>
                        <td class="py-3 px-4">
                            <span class="font-bold text-slate-900 tracking-tight">{{ trx.voucher_no || '-' }}</span>
                            <div v-if="trx.narration"
                                class="text-[10px] text-slate-400 font-normal truncate max-w-xs mt-0.5"
                                :title="trx.narration">
                                {{ trx.narration }}
                            </div>
                        </td>
                        ...
                        <td class="py-3 px-3 text-center">
                            <span
                                class="px-2 py-0.5 rounded text-[10px] font-semibold bg-slate-100 text-slate-600 border border-slate-200">
                                {{ trx.payment_mode || 'Cash' }}
                            </span>
                        </td>
                        <td class="py-3 px-4 text-center">
                            <span
                                class="px-2 py-0.5 rounded text-[10px] font-bold bg-[#e2f0d9] text-[#385723] border border-[#c5e0b4]">
                                {{ trx.status || 'Paid' }}
                            </span>
                        </td>
                    ...
                    <tr v-if="transactions.length === 0">
                        <td colspan="8" class="py-12 text-center text-slate-400 italic">
                            No {{ isPayment ? 'payment' : 'receipt' }} vouchers recorded for the selected period and
                            query scopes.
                        </td>
                    </tr>
