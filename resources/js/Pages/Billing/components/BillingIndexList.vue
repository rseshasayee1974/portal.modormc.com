...
const deleteInvoice = (invoice: any) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('billings.destroy', invoice.encrypted_id));
        }
    });
};
...
const fetchInvoiceDetails = async (data: any) => {
    ...
    try {
        const response = await axios.get(route('billings.show', encryptedId));
...
const printInvoice = (data: any) => {
    window.open(route('print.document', { module: 'billings', id: data.encrypted_id, action: 'view' }), '_blank');
};
...
<template>
    <div class="bg-white dark:bg-slate-800 shadow-xl rounded-lg border border-slate-200 dark:border-slate-700  overflow-hidden transition-all duration-300">
        <BaseDataTable
            ...
            heading="Billing Directory"
            headingIcon="DocumentTextIcon"
            showExport
            exportFilename="billing-directory-report"
        >
            <Column field="invoice_number" header="Bill #" sortable>
