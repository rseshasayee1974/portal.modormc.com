...
const submit = async () => {
    ...
    try {
        await axios.post('/settings/roles', form.value);
        ...
        if (error.response?.status === 422) {
            ...
            toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to create role' });
        }
    } finally {
            ...
            <div class="grid grid-cols-12 gap-6">
                ...
                <div class="col-span-12 md:col-span-4 flex flex-col gap-4 border-r pr-6 border-gray-100">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <BaseInput v-model="form.code" fluid placeholder="e.g. sales_mgr"  />
                    ...
                    <div class="flex flex-col gap-3 mt-4">
                        <div class="flex items-center gap-3">
                            <Checkbox v-model="form.is_system" :binary="true" inputId="is_system_create" />
                            <label for="is_system_create" class="text-xs font-bold text-gray-600">System Protected Role</label>
                        </div>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex flex-wrap gap-x-4 gap-y-2">
                                            <div v-for="perm in permissions" :key="perm.id" class="flex items-center gap-1.5 group">
                                                <Checkbox v-model="form.permissions" :inputId="'perm_create_'+perm.id" :value="perm.name"  />
                                                <label :for="'perm_create_'+perm.id" class="text-[10px] font-bold text-slate-500 group-hover:text-indigo-600 transition-colors cursor-pointer">{{ perm.action_label }}</label>
                                            </div>
                                        </div>
                                    </td>
