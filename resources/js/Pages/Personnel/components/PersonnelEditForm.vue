...
const getInitialData = () => ({
    ...
    shift_start_time: parseTime(props.personnel.shift_start_time),
    shift_end_time: parseTime(props.personnel.shift_end_time),
...
const submit = () => {
    ...
    const payload = {
        ...
        shift_start_time: formatTimeStr(form.shift_start_time),
        shift_end_time: formatTimeStr(form.shift_end_time),
...
<template>
    <div class="text-sm">
        <Tabs :value="activeTab" @update:value="handleTabUpdate">
                    ...
                    <div class="space-y-6 py-2">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Employee
                                    Code</label>
                                <div
                                    class="flex items-center h-10 px-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-800/50 text-gray-700 dark:text-gray-300 text-xs font-bold tracking-widest gap-2 select-none">
                                    <span class="pi pi-id-card text-[10px] text-indigo-400"></span>
                                    {{ form.employee_code }}
                                </div>
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">First Name
                ...
                <TabPanel value="employment">
                    <div class="space-y-6 py-2">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                            <div class="flex flex-col gap-2">
                                <label
                                    class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Department</label>
                            ...
                            <div class="flex flex-col gap-2">
                                <label
                                    class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Designation</label>
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Reporting
                                    Manager</label>
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Employment
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <DatePicker v-model="form.joining_date" dateFormat="yy-mm-dd" showIcon
                                    iconDisplay="input" placeholder="Select Date" class="w-full" />
                                <small v-if="form.errors.joining_date" class="p-error text-[10px]">{{
                                    form.errors.joining_date }}</small>
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Exit
                                    Date</label>
                                <DatePicker v-model="form.exit_date" dateFormat="yy-mm-dd" showIcon iconDisplay="input"
                                    placeholder="Select Date" class="w-full" />
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Shift Start
                                    Time</label>
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Shift End
                                    Time</label>
                ...
                <TabPanel value="finance">
                    <div class="space-y-6 py-2">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Bank IFSC
                                    Code</label>
                ...
                <TabPanel value="salary_structure">
                    <div class="space-y-6 py-2">
                        <div v-for="(struct, index) in form.salary_structures" :key="index"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                <div class="flex flex-col gap-2">
                                    <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Salary
                                        Component <span class="text-red-500">*</span></label>
                                    <BaseSelect v-model="struct.salary_component_id" :options="salaryComponentOptions"
                                        optionLabel="label" optionValue="value" placeholder="Select Component" filter
                                        class="w-full"
                                        :error="form.errors[`salary_structures.${index}.salary_component_id`]" />
                                </div>
                                <div class="flex flex-col gap-2">
                                    <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Monthly
                            ...
                            <BaseActionButton icon="pi pi-trash" severity="danger" tooltip="Remove Component Allocation"
                                class="absolute -top-2 -right-2 opacity-0 group-hover:opacity-100 transition-opacity"
                                @click="removeSalaryStructure(index)" />
                        </div>
                        <BaseButton type="button" severity="info" outlined class="w-full text-indigo-600 border-dashed"
                            label="Allocate Salary Component" icon="pi pi-plus" @click="addSalaryStructure" />
                    </div>
                </TabPanel>
                ...
                <TabPanel value="contacts">
                    <div class="space-y-6 py-2">
                        ...
                        <div v-for="(contact, index) in form.contacts" :key="index"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="flex flex-col gap-2">
                                    <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Contact
                                        Type <span class="text-red-500">*</span></label>
                                    <BaseSelect v-model="contact.contact_type" :options="contactTypeOptions"
                                        optionLabel="label" optionValue="value" placeholder="e.g. Mobile" class="w-full"
                                        :error="form.errors[`contacts.${index}.contact_type`]" />
                                </div>
                                <div class="flex flex-col gap-2">
                                    <label class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Contact
                        ...
                        <BaseButton type="button" severity="info" outlined class="w-full text-indigo-600 border-dashed"
                            label="Integrate New Communication Channel" icon="pi pi-plus" @click="addContact" />
                    </div>
                </TabPanel>
            ...
            <BaseFormActions :loading="form.processing" label="Commit Changes" cancel-label="Cancel" mode="edit"
                class="mt-8 pt-6 border-t border-gray-100 dark:border-gray-700" @cancel="emit('cancel')"
                @submit="submit" />
        </Tabs>
    </div>
</template>
