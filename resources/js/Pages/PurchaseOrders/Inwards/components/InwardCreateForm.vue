                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                        ...
                        <div v-if="form.items.length > 0" class="p-0">
                            <table class="w-full border-collapse">
                                <thead class="bg-white border-b border-slate-100">
                                    <tr>
                                        <th class="px-6 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-widest">Product Details</th>
                                        <th class="px-4 py-4 text-center text-[9px] font-black text-slate-400 uppercase tracking-widest w-36">Procurement Status</th>
                                        <th class="px-4 py-4 text-center text-[9px] font-black text-slate-400 uppercase tracking-widest w-36">Truck</th>
                                        <th class="px-4 py-4 text-right text-[9px] font-black text-slate-400 uppercase tracking-widest w-52">Full Weight with Snap</th>
                                    </tr>
                                </thead>
                                            ...
                                            <div class="flex flex-col items-end gap-2">
                                                <div class="flex items-center justify-end gap-1.5">
                                                    ...
                                                    <button 
                                                        ...
                                                        <span v-if="page.props.custom_settings?.batching?.camera == 1" class="text-[7px] font-black uppercase tracking-widest leading-none">Full</span>
                                                    </button>
                                                </div>
                                                ...
                                                <div v-if="item.loaded_weight_photo" class="relative group w-32 h-16 rounded-md overflow-hidden border border-slate-200 shadow-xs bg-slate-100">
                                                    <img :src="item.loaded_weight_photo" class="w-full h-full object-cover cursor-pointer" @click="openImageModal(item.loaded_weight_photo, 'Full Weight Snap')" />
                                                    <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5">
                                                        <button @click="openImageModal(item.loaded_weight_photo, 'Full Weight Snap')" type="button" class="p-1 bg-white/80 hover:bg-white text-slate-800 rounded text-xs" title="View Full Image">
                                                    ...
                                                    <span class="absolute bottom-0.5 right-0.5 bg-black/60 text-white text-[7px] font-bold px-1 rounded">FULL</span>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        <div v-if="form.items.length > 0" class="p-8 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
                            ...
                            <div class="flex gap-3 w-full md:w-auto">
                                <BaseButton v-if="embedded" label="Clear Items" variant="text" severity="secondary" @click="selectedPoId = null" class="!text-xs !font-bold" />
                                <BaseButton v-else label="Back to Registry" variant="text" severity="secondary" @click="router.visit(route('inwards.index'))" class="!text-xs !font-bold" />
                                <BaseButton 
