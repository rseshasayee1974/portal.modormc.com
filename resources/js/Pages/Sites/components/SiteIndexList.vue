    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs overflow-hidden">
        ...
        >
            ...
            <Column field="name" header="Site & Address" sortable>
                <template #body="{ data }">
                    <div class="flex flex-col gap-1 max-w-[420px] py-1">
                        ...
                        <!-- Address Line -->
                        <div v-if="formatFullAddress(data)" class="text-[11px] text-slate-500 dark:text-slate-400 flex items-start gap-1 leading-relaxed">
            ...
            <Column header="Customer / Patron" sortable field="patron.legal_name">
                <template #body="{ data }">
                    <div v-if="data.patrons_data && data.patrons_data.length > 0" class="flex flex-wrap gap-1 max-w-[220px]">
                        <span 
                            v-for="pat in data.patrons_data" 
                            :key="pat.id" 
                    ...
                    <span 
                        v-else-if="data.patron?.legal_name" 
                        class="text-[10px] font-semibold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md border border-slate-200/60 dark:border-slate-700 truncate max-w-[200px]"
            ...
            <Column field="plant.name" header="Parent Plant" sortable v-if="isPrivileged">
                <template #body="{ data }">
                    <div class="flex flex-col">
                        <span class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ data.plant?.name || '-' }}</span>
                        <span v-if="data.plant?.code" class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">{{ data.plant?.code }}</span>
                    </div>
                </template>
            </Column>
                ...
                <template #body="{ data }">
                    <div class="flex items-center gap-1.5 justify-center">
                        <span 
                            ...
                            :class="(data.status === 'Active' || !data.status) ? 'bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.4)]' : 'bg-rose-500'"
                        ></span>
                        <Tag 
            ...
            <Column header="Actions" class="text-right" headerClass="text-right" style="width: 100px">
                <template #body="{ data }">
                    <div class="flex items-center justify-end gap-1">
                        ...
                        <!-- Delete Button -->
                        <Button 
                            v-if="isSuperAdmin || !data.is_in_use"
                            icon="pi pi-trash" 
                            text 
                            rounded 
                            size="small" 
                            severity="danger" 
                            class="!hover:bg-red-50 !w-7 !h-7"
                            title="Delete Site"
            ...
            <template #expansion="{ data }">
                ...
                <div v-else class="p-8 flex flex-col items-center justify-center bg-slate-50/50 dark:bg-slate-900/20">
                    <div class="w-8 h-8 rounded-full border-2 border-slate-200 border-t-indigo-600 animate-spin mb-2"></div>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest animate-pulse">Loading Site Form...</p>
                </div>
            </template>
            ...
            <template #empty>
                <div class="py-12 flex flex-col items-center justify-center text-slate-400 text-center">
                    ...
                    <span class="text-[11px] text-slate-400 mt-0.5">Use the registration form above to create your first operational delivery site.</span>
                </div>
            </template>
        </BaseDataTable>
    </div>
