    ...
    <div class="container">
                        ...
                        @php
                            $cleanLogoPath = ltrim(
                                str_replace(
                                    ...
                                    '',
                                    $batch->workOrder->plant->logo_path,
                                ),
                                '/',
                            );
                ...
                <td>
                    ...
                    <table class="details-grid">
                        ...
                        <tr>
                            ...
                            <td class="grid-value" style="font-family: monospace;">
                                {{ $batch->workOrder?->mixDesign?->design_code ?? '-' }}</td>
                        </tr>
                        <tr>
        ...
        @if ($batch->materials->count() > 0)
            ...
            <table class="materials-table">
                ...
                <tbody>
                    @php
                        $groupedIngredients = $batch->materials->groupBy(function($mat) {
                            return $mat->product_id ?? $mat->material_name;
                        })->map(function($group) {
                            $first = $group->first();
                            $target = $group->sum('target_qty');
                        ...
                        $ingSno = 1;
                    @endphp
                    @foreach ($groupedIngredients as $mat)
                        @php
                        ...
                        <tr class="secondary-row">
                            <td>{{ $ingSno++ }}</td>
                            <td style="font-weight: 700;">{{ $mat->material_name }}</td>
                            <td>{{ $mat->uom_code }}</td>
            ...
            <tr>
                ...
                <td>
                    <div style="min-height: 45px; text-align: center; margin-bottom: 5px;">
                        @if ($batch->workOrder?->plant?->seal_sign_path)
                            @php
                                $sealPath = ltrim(
                                    str_replace(
                                        ['public/', 'storage/', '/storage/'],
                                        '',
                                        $batch->workOrder->plant->seal_sign_path,
                                    ),
                                    '/',
                                );
