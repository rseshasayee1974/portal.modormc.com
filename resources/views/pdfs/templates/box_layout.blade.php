...
<div class="inv-root">
            ...
            <table>
                <tr>
                    <td style="vertical-align: top; line-height: 1.4;">
                        @if(!empty($data['meta']['irn'])) <div>IRN : <strong>{{ $data['meta']['irn'] }}</strong></div> @endif
                        @if(!empty($data['meta']['ack_no'])) <div>Ack No. : <strong>{{ $data['meta']['ack_no'] }}</strong></div> @endif
                        @if(!empty($data['meta']['ack_date'])) <div>Ack Date : <strong>{{ $data['meta']['ack_date'] }}</strong></div> @endif
                    </td>
                    @if(!empty($data['meta']['qr_code']))
                        <td style="width: 80px; text-align: right; vertical-align: top;">
                            <img src="{{ $data['meta']['qr_code'] }}" style="max-height: 75px; max-width: 75px; object-fit: contain;" />
                        </td>
                    @endif
                </tr>
            </table>
    ...
    <table class="items-table">
        <thead>
            <tr>
                ...
                @if($pdfSettings['description'] ?? true) <th style="width: 20%;">Description</th> @endif
                @if($pdfSettings['show_pump_charges'] ?? false) <th style="width: 10%;">Op. Type</th> @endif
                @if($pdfSettings['qty'] ?? true) <th style="width: 10%;">QTY.in CU.M</th> @endif
                @if($pdfSettings['unit'] ?? true) <th style="width: 10%;">Unit Price</th> @endif
                @if($pdfSettings['show_pump_charges'] ?? false) <th style="width: 10%;">Pump Chg</th> @endif
                <th style="width: 12%;">Taxable Amount</th>
                @if(($pdfSettings['tax_rate'] ?? true) || ($pdfSettings['tax_amount'] ?? true)) <th style="width: 18%;">Tax</th> @endif
    ...
    ACCOUNT NUMBER: <strong style="letter-spacing: 1px;">{{ $data['company']['bank']['account_number'] }}</strong><br>
