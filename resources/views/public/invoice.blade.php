...
<body class="bg-slate-50 min-h-screen flex flex-col antialiased">
    ...
    <header class="no-print sticky top-0 z-50 bg-[#1d2d3e] text-white border-b border-[#2c3e50] shadow-md">
        <div class="max-w-6xl mx-auto px-4 py-3 sm:px-6 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <!-- Download PDF Button -->
                <a href="{{ route('public.invoice.pdf', ['token' => $token]) }}" class="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 shadow-sm">
                    <!-- Download Icon -->
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div class="no-print bg-amber-50 border-b border-amber-200 py-2.5 px-4 text-center">
        <div class="max-w-6xl mx-auto flex items-center justify-center gap-2 text-xs font-medium text-amber-800">
            ...
            <span>You are viewing a secure read-only version of this invoice. No authentication required.</span>
        </div>
    </div>

    <!-- Main Content Layout -->
    <main class="flex-1 py-8 px-4 sm:px-6">
        <div class="print-container max-w-4xl mx-auto bg-white shadow-xl border border-slate-200 rounded-xl overflow-hidden p-6 sm:p-10 transition-all duration-300">
