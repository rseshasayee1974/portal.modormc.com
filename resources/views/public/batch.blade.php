...
<head>
    ...
    <title>Batch Sheet Report h</title>
    <!-- Google Fonts -->
...
<body class="bg-slate-50 min-h-screen flex flex-col antialiased">
    ...
    <header class="no-print sticky top-0 z-50 bg-[#1d2d3e] text-white border-b border-[#2c3e50] shadow-md">
        <div class="max-w-7xl mx-auto px-4 py-3 sm:px-6 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <!-- Download PDF Button -->
                <a href="{{ route('public.batch.pdf', ['token' => $token]) }}" class="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 shadow-sm">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div class="no-print bg-amber-50 border-b border-amber-200 py-2.5 px-4 text-center">
        <div class="max-w-7xl mx-auto flex items-center justify-center gap-2 text-xs font-medium text-amber-800">
            ...
            <span>You are viewing a secure read-only version of this batch sheet report. No authentication required.</span>
        </div>
    </div>

    <!-- Main Content Layout (Landscape optimized padding) -->
    <main class="flex-1 py-8 px-4 sm:px-6">
        <div class="print-container max-w-7xl mx-auto bg-white shadow-xl border border-slate-200 rounded-xl overflow-hidden p-6 sm:p-10 transition-all duration-300">
            <!-- Include the dynamic layout view directly -->
            @include('pdfs.batches.batch_sheet', ['batch' => $batch, 'sheet' => $sheet, 'isPreview' => true])
        </div>
    </main>

    <!-- Simple Footer -->
    <footer class="no-print bg-slate-950 text-slate-500 py-6 px-4 text-center text-xs border-t border-slate-900">
        <p>&copy; {{ date('Y') }} Secured by 64-character encrypted tokens.</p>
    </footer>
