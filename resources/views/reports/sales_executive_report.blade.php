...
<head>
    ...
    <style>
        {!! $css ?? $report_css ?? (file_exists(public_path('css/reports/sales_executive_report.css')) ? file_get_contents(public_path('css/reports/sales_executive_report.css')) : (file_exists(public_path('css/reports/report_pdf.css')) ? file_get_contents(public_path('css/reports/report_pdf.css')) : '')) !!}
    </style>
</head>
<body>
        ...
        <tr>
            <td style="width: 52%;">
                <div class="address-box" style="margin-top: 5px;">
                    ...
                    <span class="address-name">Sales Executive Performance Report</span>
                    <span>Consolidated dispatch volume, batch sizes, truck weights, and billing totals</span>
                </div>
            </td>
    ...
    <div class="statement-title-container">
        <h2 class="statement-title">Sales Executive Consolidated Report</h2>
        <span class="statement-period">Reporting Period: {{ $start }} to {{ $end }}</span>
    </div>

    <!-- Section 1: Consolidated by Sales Executive -->
    <table class="data-table">
        <thead>
            <tr>
                <th width="5%">#</th>
                <th width="40%">Sales Executive</th>
                <th width="15%">Code</th>
                <th width="12%">Trips</th>
                <th width="14%">Batch Size</th>
                <th width="14%">Delivered Qty</th>
            </tr>
        </thead>
        <tbody>
            @php 
            ...
            @foreach($rows as $index => $row)
                <tr>
                    ...
                    <td class="font-bold">{{ $row['sales_executive_name'] ?? $row['executive_name'] }}</td>
                    <td class="text-center">{{ $row['executive_code'] ?: '-' }}</td>
    ...
    @if(!empty($executive_customer_summary) && count($executive_customer_summary) > 0)
    ...
    <div class="statement-title-container" style="margin-top: 20px; border-bottom: 1.5px solid #cbd5e1; padding-bottom: 3px;">
        <h3 style="font-size: 10pt; font-weight: bold; color: #334155; margin: 0; text-transform: uppercase;">Executive & Customer Volume Summary</h3>
    </div>
    ...
    <table class="data-table">
        <thead>
            <tr>
                <th width="5%">#</th>
                <th width="35%">Sales Executive</th>
                <th width="35%">Customer / Account</th>
                <th width="11%">Trips</th>
                <th width="14%">Delivered Qty</th>
            </tr>
        </thead>
        <tbody>
            @foreach($executive_customer_summary as $index => $row)
                <tr>
                    <td class="text-center">{{ $index + 1 }}</td>
                    <td class="font-bold">{{ $row['sales_executive_name'] }}</td>
    ...
    <!-- Section 3: Sales Executive Batching / Trip Verification List -->
    <div class="statement-title-container" style="margin-top: 25px; border-bottom: 2px solid #0284c7; padding-bottom: 4px;">
        ...
        <span style="font-size: 8.5pt; color: #64748b;">Itemized sales executive trip dispatch tickets for audit and verification</span>
    </div>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th width="14%">Sales Executive</th>
        ...
        <tbody>
            @foreach($transactions as $index => $trip)
                <tr>
                    ...
                    <td class="font-bold">{{ $trip['sales_executive_name'] ?? '-' }}</td>
            ...
            @endforeach
            <tr class="total-row">
                <td colspan="8" class="text-center font-bold">Total Executive Dispatches ({{ count($transactions) }} Trips)</td>
