...
<head>
    ...
    <style>
        {!! $css ?? $report_css ?? (file_exists(public_path('css/reports/sales_register_pdf.css')) ? file_get_contents(public_path('css/reports/sales_register_pdf.css')) : (file_exists(public_path('css/reports/report_pdf.css')) ? file_get_contents(public_path('css/reports/report_pdf.css')) : '')) !!}
    </style>
</head>
<body>
        ...
        <tbody>
            ...
            @forelse($items as $idx => $item)
                ...
                <tr>
                    ...
                    <td class="text-center">{{ \Carbon\Carbon::parse($item['invoice_date'])->format('d-m-Y') }}</td>
                    <td class="font-bold">{{ $item['customer_name'] }}</td>
            ...
            @if(!empty($items))
                <tr class="total-row">
                    <td colspan="6" class="text-center font-bold">TOTAL SALES</td>
