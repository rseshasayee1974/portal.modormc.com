        ...
        $batches = DB::table('mm_batches as b')
            ...
            ->map(function ($row) {
                ...
                return [
                    ...
                    'has_eway_bill' => !empty($row->eway_bill_no),
                    'sales_order' => [
   ...
   public function store(StoreBatchRequest $request)
{
    ...
    public function show($batch, Request $request)
    {
        ...
        $responseData = [
            ...
            'load_total_amount' => (float)($primaryDispatch['load_total_amount'] ?? 0),
            'customer_id' => $batchRow->customer_id,
            'customer_name' => $batchRow->customer_legal_name,
            'site_id' => $batchRow->site_id,
            'site_name' => $batchRow->site_name,
            'mix_design_id' => $batchRow->mix_design_id,
            'mix_design_name' => $batchRow->mix_design_name,
            'mix_design_code' => $batchRow->mix_design_code,
            'truck_registration' => $primaryDispatch['truck']['registration'] ?? null,
            'invoice_id' => $primaryDispatch['status']['invoice_id'] ?? null,
            'invoice_number' => $primaryDispatch['status']['invoice_number'] ?? ($primaryDispatch['status']['invoice']['full_number'] ?? null),
            'invoice_prefix' => $primaryDispatch['status']['invoice']['invoice_prefix'] ?? null,
            'has_invoice' => !empty($primaryDispatch['status']['invoice_id']),
            'einvoice_irn' => $primaryDispatch['status']['invoice']['einvoice_irn'] ?? null,
            'einvoice_status' => $primaryDispatch['status']['invoice']['einvoice_status'] ?? null,
            'has_einvoice' => !empty($primaryDispatch['status']['invoice']['einvoice_irn']) || ($primaryDispatch['status']['invoice']['einvoice_status'] ?? null) === 'generated',
            'eway_bill_no' => $primaryDispatch['status']['invoice']['eway_bill_no'] ?? null,
            'has_eway_bill' => !empty($primaryDispatch['status']['invoice']['eway_bill_no']),
            'sales_order' => [
    ...
    private function loadBatchForToken(Batch $batch): void
    {
        ...
        $batch->load([
            ...
            'salesOrder.mixDesign:id,concrete_grade_id,design_name,design_code,design_type',
    ...
    public function gatePass(Batch $batch)
    {
        $batch->load([
            ...
            'workOrder.mixDesign:id,concrete_grade_id,design_name,design_code,design_type',
