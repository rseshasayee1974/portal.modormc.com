...
use App\Models\SalesOrder; // This is the new SalesOrder (formerly WorkOrder)
use App\Models\CustomerPO; // This is the new CustomerPO (formerly SalesOrder)
...
class SalesOrderIndexDataFactory
{
    ...
    private function schemaFlags(): array
    {
        ...
        return [
            'hasPlantIdColumn' => Schema::hasColumn('mm_sales_orders', 'plant_id'),
            'hasPrefixColumn' => Schema::hasColumn('mm_sales_orders', 'prefix'),
            'hasOrderNoColumn' => Schema::hasColumn('mm_sales_orders', 'order_no'),
            'hasTotalQtyColumn' => Schema::hasColumn('mm_sales_orders', 'total_qty'),
            'hasProducedQtyColumn' => Schema::hasColumn('mm_sales_orders', 'produced_qty'),
            'hasCustomerIdColumn' => Schema::hasColumn('mm_sales_orders', 'customer_id'),
            'hasSiteIdColumn' => Schema::hasColumn('mm_sales_orders', 'site_id'),
            'hasMixDesignIdColumn' => Schema::hasColumn('mm_sales_orders', 'mix_design_id'),
            'hasCreatedAtColumn' => Schema::hasColumn('mm_sales_orders', 'created_at'),
            'hasLegacyCreatedColumn' => Schema::hasColumn('mm_sales_orders', 'created'),
            ...
            'hasBatchesRelation' => Schema::hasTable('mm_batches') && Schema::hasColumn('mm_batches', 'sales_order_id'),
            'hasDispatchesRelation' => Schema::hasTable('mm_dispatches') && Schema::hasColumn('mm_dispatches', 'sales_order_id'),
        ];
    }

    private function buildWorkOrdersQuery(?int $activePlantId, array $schema): Builder
    {
        $query = SalesOrder::query()->select('mm_sales_orders.*');
        ...
        if (!$schema['hasPrefixColumn']) {
            $query->addSelect(DB::raw("'SO' as prefix"));
        }
        if (!$schema['hasOrderNoColumn']) {
            $query->addSelect(DB::raw("COALESCE(work_order_number, CONCAT('SO-', id)) as order_no"));
        }
