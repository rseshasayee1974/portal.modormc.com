...
class PrintDataFormatter
{
    ...
    public static function resolveTermsCondition(array $settings, $orderType, int $plantId, ?string $fallbackTerms = null): string
    {
        ...
        if (!empty($settings['pdf']['terms_text'])) {
            $text = (string) $settings['pdf']['terms_text'];
        } else {
            ...
            foreach ($orderTypes as $type) {
                ...
                if ($tc && !empty(trim(strip_tags($tc->terms_condition ?? '')))) {
                    $text = (string) $tc->terms_condition;
                    break;
                }
            }
            if (empty($text)) {
        ...
        return self::cleanTermsWhitespace($text);
    }

    public static function cleanTermsWhitespace(?string $content): string
    {
        ...
        // If content contains HTML tags, clean up empty paragraphs, empty breaks, and outer whitespace
        if ($trimmed !== strip_tags($trimmed)) {
            $trimmed = preg_replace('/<p[^>]*>(\s|&nbsp;|<br\s*\/?>)*<\ /p>/i', '', $trimmed);
    $trimmed = preg_replace('/<div[^>]*>(\s|&nbsp;|<br\s*\ /?>)*<\ /div>/i', '', $trimmed);
                $trimmed = preg_replace('/<span[^>]*>(\s|&nbsp;|<br\s*\ /?>)*<\ /span>/i', '', $trimmed);
                            $trimmed = preg_replace('/(<br\s*\ /?>\s*){3,}/i', '<br><br>', $trimmed);
                                return trim($trimmed);
                                }

                                // For plain text: trim each line and avoid more than 1 consecutive blank line
                                $lines = explode("\n", str_replace(["\r\n", "\r"], "\n", $trimmed));
                                $cleanedLines = [];
                                $lastWasEmpty = false;
                                ...
                                if ($primaryBank) {
                                return [
                                'account_name' => $primaryBank->account_name ?? ($legalEntity?->legal_name ??
                                $legalEntity?->entity_name ?? ''),
                                ...
                                $stateCode = $plAddr?->state?->state_code ?? ($plant?->gstin && strlen($plant->gstin) >=
                                2 ? substr($plant->gstin, 0, 2) : '');
                                $gstin = $plant?->gstin ?? '';
                                $pan = strlen($gstin) >= 14 ? substr($gstin, 2, 10) : ($plant?->pan ?? '');

                                return [
                                'name' => $plant?->entity?->legal_name ?? $plant?->entity?->entity_name ?? $plant?->name
                                ?? 'Company',
                                ...
                                if (!$partner) {
                                return [
                                'name' => 'N/A', 'address' => '', 'city' => '', 'state' => '', 'pin' => '', 'gstin' =>
                                '', 'phone' => '',
                                ];
                                }
                                ...
                                $primaryContact = $partner->contacts->where('is_primary', true)->first() ??
                                $partner->contacts->first();
                                if ($primaryContact && isset($primaryContact->addresses)) {
                                $partnerAddr = $primaryContact->addresses->where('is_primary', true)->first() ??
                                $primaryContact->addresses->first();
                                }
                                }
                                ...
                                public static function resolveTaxDetails($taxModel, bool $isIntra, float $priceTax,
                                float $subtotal): array
                                {
                                ...
                                if ($taxRate <= 0 && $priceTax> 0 && $subtotal > 0) {
                                    ...
                                    $taxName = $taxGroup . ' ' . ($taxRate == floor($taxRate) ? (int)$taxRate :
                                    $taxRate) . '%';
                                    }
                                    ...
                                    public static function compileTaxLines($items, bool $isIntra, string $taxField,
                                    $subtotalFieldOrFn): array
                                    {
                                    $taxLines = [];
                                    ...
                                    if ($priceTax <= 0 && !$taxModel) { continue; } if ($taxRate <=0 && $subtotal> 0) {
                                        $taxGroup = $isIntra ? 'GST' : 'IGST';
                                        }
                                        if (empty($taxGroup)) {
                                        $taxGroup = $isIntra ? 'GST' : 'IGST';
                                        ...
                                        return collect($taxLines)->map(fn($amt, $lbl) => ['label' => $lbl, 'amount' =>
                                        $amt])->values()->toArray();
                                        }

                                        public static function formatMixDesignDescription(?string $baseDesc,
                                        $mixDesign): string
                                        {
                                        ...
                                        {
                                        $order->loadMissing(['items.product', 'items.uom', 'items.tax', 'vendor',
                                        'plant', 'plant.entity', 'plant.addresses', 'currency']);
                                        $data = self::base();
                                        ...
                                        $templateKey);
                                        $data['doc_title'] = $data['settings']['pdf']['labels']['invoice_title'] ??
                                        'PURCHASE ORDER';
                                        ...
                                        $taxDetails = self::resolveTaxDetails($item->tax, $isIntra,
                                        (float)$item->price_tax, (float)$item->price_subtotal);
                                        return [
                                        'no' => $idx + 1, 'name' => $item->product->title ?? '', 'description' =>
                                        $item->description ?? '',
                                        'hsn' => $item->product->hsn_code ?? '-', 'qty' =>
                                        (float)$item->product_quantity,
                                        'received_qty' => (float)($item->received_quantity ?? 0), 'unit' =>
                                        $item->uom->unit_code ?? '',
                                        'unit_price' => (float)$item->unit_price, 'tax_name' => $taxDetails['name'] ?:
                                        '-', 'tax_rate' => $taxDetails['rate'],
                                        'tax_group' => $taxDetails['group'], 'tax_amount' => (float)$item->price_tax,
                                        'total' => (float)$item->price_total,
                                        ];
                                        })->toArray();

                                        $data['totals'] = [
                                        'sub_total' => (float)$order->amount_untaxed, 'discount' =>
                                        (float)($order->discount_amount ?? 0),
                                        'tax_lines' => self::compileTaxLines($order->items, $isIntra, 'price_tax',
                                        'price_subtotal'),
                                        'shipping' => (float)($order->shipping_charges ?? 0), 'adjustment' =>
                                        (float)($order->adjustment ?? 0),
                                        'round_off' => (float)($order->round_off ?? 0), 'grand_total' =>
                                        (float)$order->amount_total,
                                        ];
                                        $data['meta'] = [
                                        'po_number' => $order->po_number ?? $order->ref_no, 'project_name' =>
                                        $order->plant->name,
                                        'currency_code' => $order->currency->currency_code ?? 'INR', 'currency_symbol'
                                        => $order->currency->currency_symbol ?? '₹',
                                        'notes' => $order->notes ?? '',
                                        'terms_text' => self::resolveTermsCondition($data['settings'], 'Purchase Order',
                                        $order->plant_id, $order->terms_conditions ?? ''),
                                        'total_words' => self::numberToWords($order->amount_total,
                                        $order->currency->currency_code ?? 'INR'),
                                        'site_incharge' => $order->plant->site_incharge ?? '', 'contact_no' =>
                                        $order->plant->contact_no ?? '',
                                        ...
                                        {
                                        $invoice->loadMissing(['plant', 'plant.entity', 'plant.addresses', 'partner',
                                        'partner.addresses', 'partner.contacts.addresses', 'items.tax', 'items.uom',
                                        'items.itemTaxes', 'orderTaxes']);
                                        $data = self::base();
                                        $data['id'] = $invoice->id;
                                        $data['invoice_id'] = $invoice->id;
                                        $data['invoice'] = $invoice;
                                        $data['settings'] = self::getCustomSettings($invoice->plant_id, 'invoices');
                                        $defaultTitle = $invoice->invoice_type === 'bill' ? 'PURCHASE BILL' : 'TAX
                                        INVOICE';
                                        $docTitle = $data['settings']['pdf']['labels']['invoice_title'] ??
                                        $defaultTitle;
                                        if (!empty($invoice->invoice_label)) {
                                        if (strtolower($invoice->invoice_label) === 'manual') $docTitle = 'MANUAL
                                        BILLING';
                                        elseif (strtolower($invoice->invoice_label) === 'dispatch') $docTitle =
                                        $defaultTitle;
                                        else $docTitle = strtoupper($invoice->invoice_label);
                                        }
                                        $data['doc_title'] = $docTitle;
                                        $data['doc_no'] = $invoice->prefix . $invoice->invoice_number;
                                        $data['doc_date'] = $invoice->invoice_date?->format('d/m/Y') ??
                                        now()->format('d/m/Y');
                                        ...
                                        $dispatch = \App\Models\Dispatch::whereHas('status', function ($q) use
                                        ($invoice) {
                                        $q->where('invoice_id', $invoice->id);
                                        })->with([
                                        'salesOrder.customer', 'salesOrder.site', 'salesOrder.salesExecutive',
                                        ...
                                        $partner = $dispatch->customer ?: ($dispatch->salesOrder?->customer ?:
                                        ($dispatch->customerPO?->patron ?: null));
                                        }
                                        }

                                        $customerPO = null;
                                        if (!empty($invoice->ref_id) && is_numeric($invoice->ref_id)) {
                                        $customerPO = \App\Models\CustomerPO::with(['patron',
                                        'site'])->find($invoice->ref_id);
                                        ...
                                        $salesOrder = null;
                                        if (!empty($invoice->ref_id) && is_numeric($invoice->ref_id)) {
                                        $salesOrder = \App\Models\SalesOrder::with(['customer', 'site',
                                        'salesExecutive', 'mixDesign'])->find($invoice->ref_id);
                                        ...
                                        $site = null;
                                        if ($dispatch) {
                                        $site = $dispatch->unloadSite ?: ($dispatch->salesOrder?->site ?:
                                        ($dispatch->customerPO?->site ?: null));
                                        }
                                        if (!$site && $customerPO) {
                                        ...
                                        \App\Models\CustomerPO::with(['site'])->find($invoice->ref_id);
                                        if ($customerPO) {
                                        $site = $customerPO->site;
                                        } else {
                                        $salesOrder = $salesOrder ?:
                                        \App\Models\SalesOrder::with(['site'])->find($invoice->ref_id);
                                        if ($salesOrder) {
                                        $site = $salesOrder->site;
                                        }
                                        }
                                        ...
                                        $salesExec = $dispatch?->salesExecutive ?: ($salesOrder?->salesExecutive ?:
                                        null);
                                        $salesPersonName = '';
                                        if ($salesExec) {
                                        $salesPersonName = trim(($salesExec->first_name ?? '') . ' ' .
                                        ($salesExec->last_name ?? ''));
                                        }
                                        ...
                                        $pumpName = $dispatch->concretePump->registration ?:
                                        ($dispatch->concretePump->name ?? 'Dumping');
                                        ...
                                        $driverName = $dispatch?->driver ? trim(($dispatch->driver->first_name ?? '') .
                                        ' ' . ($dispatch->driver->last_name ?? '')) : '';

                                        $carrierDriverParts = array_filter([$transportName, $truckReg, $driverName]);
                                        $carrierDriverText = !empty($carrierDriverParts) ? implode(' , ',
                                        $carrierDriverParts) : '-';

                                        $isIntra = self::isIntraState($invoice->plant->gstin ?? '', $partner?->gstin ??
                                        '');

                                        $showPumpCharges = !isset($data['settings']['pdf']['show_pump_charges']) ||
                                        $data['settings']['pdf']['show_pump_charges'];
                                        ...
                                        $showPumpCharges, $dispatch, $dispatchPumpCharge, &$pumpChargesTotal) {
                                        $taxModel = $item->tax;
                                        $lineTaxAmount = (float)$item->line_tax_amount;

                                        // Fallback: if no direct tax relationship, derive from itemTaxes (order_taxes
                                        splits)
                                        if (!$taxModel && $item->relationLoaded('itemTaxes') &&
                                        $item->itemTaxes->isNotEmpty()) {
                                        ...
                                        $taxGroup = 'IGST';
                                        } elseif (str_contains($firstSplitName, 'cgst') || str_contains($firstSplitName,
                                        'sgst')) {
                                        $taxGroup = 'GST';
                                        } else {
                                        $taxGroup = $isIntra ? 'GST' : 'IGST';
                                        }
                                        $taxName = $taxGroup . ' ' . ($totalRate == floor($totalRate) ? (int)$totalRate
                                        : $totalRate) . '%';
                                        $taxDetails = ['rate' => $totalRate, 'group' => $taxGroup, 'name' => $taxName];
                                        } else {
                                        $taxDetails = self::resolveTaxDetails($taxModel, $isIntra, $lineTaxAmount,
                                        (float)$item->subtotal);
                                        }
                                        ...
                                        if (is_numeric($dispatch->concrete_pump)) {
                                        $pumpMachine = \App\Models\Machine::find($dispatch->concrete_pump);
                                        $operationType = $pumpMachine?->registration ??
                                        (string)$dispatch->concrete_pump;
                                        } else {
                                        ...
                                        return [
                                        'no' => $idx + 1, 'name' => $item->item_name, 'description' => '', 'hsn' =>
                                        $item->hsn_code ?? '-',
                                        'qty' => (float)$item->quantity, 'unit' => $item->uom->unit_code ?? 'm³',
                                        'unit_price' => (float)$item->price_unit,
                                        'operation_type' => $operationType, 'pump_charge' => $pumpCharge,
                                        'tax_name' => $taxDetails['name'] ?: '-', 'tax_rate' => $taxDetails['rate'],
                                        'tax_group' => $taxDetails['group'], 'tax_amount' => $lineTaxAmount,
                                        'total' => (float)($item->line_total ?? ($item->quantity * $item->price_unit)),
                                        ];
                                        })->toArray();

                                        $taxLines = $invoice->orderTaxes->map(function($ot) { return ['label' =>
                                        $ot->name, 'amount' => (float)$ot->amount]; })->toArray();
                                        $data['totals'] = [
                                        'sub_total' => (float)$invoice->subtotal, 'discount' =>
                                        (float)($invoice->discount_total ?? $invoice->global_discount),
                                        'tax_lines' => $taxLines, 'shipping' => (float)$invoice->shipping_charges,
                                        'adjustment' => (float)$invoice->adjustment,
                                        'round_off' => (float)$invoice->round_off,
                                        'grand_total' => (float)$invoice->total_amount,
                                        ];
                                        $orderTypeForTerms = $invoice->invoice_type === 'bill' ? 'Purchase Bill' :
                                        [($invoice->invoice_label ?? 'Tax Invoice'), 'Tax Invoice'];
                                        ...
                                        $testDummyQrPath =
                                        asset('storage/plants/demo-mining-corp/parker-llc-plant/upi_qr_1784092752.png');

                                        $data['meta'] = [
                                        'currency_code' => 'INR',
                                        'currency_symbol' => '₹',
                                        'notes' => $invoice->notes ?? '',
                                        'terms_text' => self::resolveTermsCondition($data['settings'],
                                        $orderTypeForTerms, $invoice->plant_id, "1. Goods once sold will not be taken
                                        ...
                                        str_starts_with($invoice->einvoice_qr_code, 'data:image') ||
                                        str_starts_with($invoice->einvoice_qr_code, 'http')
                                        ? $invoice->einvoice_qr_code
                                        : asset('storage/' . ltrim(str_replace(['public/', 'storage/', '/storage/'], '',
                                        $invoice->einvoice_qr_code), '/'))
                                        ) : (
                                        ...
                                        'acc_no' => $partner?->account_number ?? $partner?->code ?? ($partner ? 'AC-' .
                                        $partner->id : ''),
                                        ...
                                        public static function fromQuotation($quotation, ?array $customSettings = null):
                                        array
                                        {
                                        $statusLabels = [
                                        0 => 'DRAFT',
                                        ...
                                        public static function fromCustomerPO($customerPO, ?array $customSettings =
                                        null): array
                                        {
                                        $statusLabels = [
                                        0 => 'DRAFT',
                                        ...
                                        private static function fromQuotationOrCustomerPO($model, string $module, string
                                        $defaultTitle, string $docNo, ?string $docDate, ?string $dueDate, string $state,
                                        ?array $customSettings = null): array
                                        {
                                        $model->loadMissing([
                                        'items.mixDesign',
                                        ...
                                        $data = self::base();
                                        $data['settings'] = $customSettings ?? self::getCustomSettings($model->plant_id,
                                        $module);
                                        if (!$customSettings && $module === 'customer_pos' &&
                                        empty(\App\Models\CustomSetting::getForModule($model->plant_id,
                                        'customer_pos'))) {
                                        ...
                                        $settings = \App\Models\CustomSetting::getForModule($model->plant_id,
                                        'batching');
                                        ...
                                        $isBoom = str_contains(strtolower($model->concretePump->vehicle_type ?? ''),
                                        'boom')
                                        || str_contains(strtolower($model->concretePump->registration ?? ''), 'boom');
                                        $isManual = str_contains(strtolower($model->concretePump->vehicle_type ?? ''),
                                        'manual')
                                        ...
                                        $selectedRate = $isBoom ? (float)($model->boom_pump_rate ?? 0) :
                                        (float)($model->pump_rate ?? 0);
                                        }
                                        } else {
                                        $selectedRate = (float)($model->manual_rate ?? 0);
                                        }
                                        ...
                                        $data['items'] = $model->items->map(function ($item, $idx) use ($model,
                                        $isIntra, $isTaxInclusive, $selectedRate, &$subtotalAmt, &$totalTaxAmt,
                                        &$grandTotalAmt) {
                                        ...
                                        $taxModel = $item->tax;
                                        $taxRate = $taxModel ? (float)($taxModel->tax_rate ?? $taxModel->rate ?? 0) :
                                        0.0;
                                        ...
                                        $taxDetails = self::resolveTaxDetails($taxModel, $isIntra, $lineTax,
                                        $lineUntaxed);
                                        $unitPrice = $isTaxInclusive ? (float)($qty > 0 ? ($lineUntaxed / $qty) : $rate)
                                        : $rate;
                                        ...
                                        if ($showPumpCharges) {
                                        $displayUnitPrice = $unitPrice - ($isTaxInclusive ? (float)($actualItemPumpRate
                                        / (1 + $taxRate / 100)) : (float)$actualItemPumpRate);
                                        ...
                                        $itemDescription = self::formatMixDesignDescription($item->description,
                                        $item->mixDesign);
                                        ...
                                        $formattedTaxLines = collect($taxLines)->map(fn($amt, $lbl) => ['label' => $lbl,
                                        'amount' => $amt])->values()->toArray();
                                        ...
                                        $termsText = self::resolveTermsCondition($data['settings'], ($module ===
                                        'customer_pos' ? 'Customer PO' : 'Quotation'), $model->plant_id,
                                        ...
                                        $data = self::base();
                                        $data['settings'] = self::getCustomSettings($salesOrder->plant_id,
                                        'sales_orders') ?: self::getCustomSettings($salesOrder->plant_id, 'quotations');
                                        $data['doc_title'] = $data['settings']['pdf']['labels']['invoice_title'] ??
                                        'SALES ORDER';
                                        $data['doc_no'] = ($salesOrder->prefix ?? '') . ($salesOrder->order_no ??
                                        $salesOrder->id);
                                        $data['doc_date'] = $salesOrder->created_at ?
                                        $salesOrder->created_at->format('d/m/Y') : now()->format('d/m/Y');
                                        $data['due_date'] = $salesOrder->scheduled_end ?
                                        \Carbon\Carbon::parse($salesOrder->scheduled_end)->format('d/m/Y') : '';
                                        $statusMap = [1 => 'SCHEDULED', 2 => 'IN PROGRESS', 3 => 'COMPLETED', 4 =>
                                        'CANCELLED'];
                                        ...
                                        $isTaxInclusive = false; if ($salesOrder->customerPO) $isTaxInclusive =
                                        (bool)($salesOrder->customerPO->is_tax_inclusive ?? false);
                                        $isIntra = self::isIntraState($salesOrder->plant->gstin ?? '',
                                        $salesOrder->customer?->gstin ?? '');
                                        $quotationItems = $salesOrder->customerPO?->quotation?->items ?? collect();
                                        if ($quotationItems->isNotEmpty()) {
                                        $data['items'] = $quotationItems->map(function ($item, $idx) use ($isIntra,
                                        $isTaxInclusive, $salesOrder) {
                                        $subtotal = (float)($item->untaxed_amount ?? ($item->quantity * $item->rate));
                                        $taxDetails = self::resolveTaxDetails($item->tax, $isIntra,
                                        (float)$item->tax_amount, $subtotal);
                                        $description = self::formatMixDesignDescription($item->description,
                                        $item->mixDesign);
                                        $unitPrice = $isTaxInclusive ? (float)($item->quantity > 0 ? ($subtotal /
                                        $item->quantity) : $item->rate) : (float)$item->rate;
                                        return [
                                        'no' => $idx + 1, 'name' => $item->mixDesign?->design_name ?? 'N/A',
                                        'description' => $description,
                                        'hsn' => $item->mixDesign?->hsn_code ?? '-', 'qty' => (float)$item->quantity,
                                        'received_qty'=> (float)($salesOrder->produced_qty ?? 0),
                                        'unit' => $item->mixDesign?->unit?->unit_code ?? 'm³', 'unit_price' =>
                                        $unitPrice, 'tax_name' => $taxDetails['name'] ?: '-',
                                        'tax_rate' => $taxDetails['rate'], 'tax_group' => $taxDetails['group'],
                                        'tax_amount' => (float)$item->tax_amount, 'total' => (float)($item->amount_total
                                        ?? ($item->quantity * $item->rate)),
                                        ];
                                        })->toArray();
                                        } else {
                                        $mixDesign = $salesOrder->mixDesign; $qty = (float)($salesOrder->total_qty ??
                                        0);
                                        $poItem = null; if ($salesOrder->customerPO) $poItem =
                                        $salesOrder->customerPO->items->where('mix_design_id',
                                        $salesOrder->mix_design_id)->first();
                                        $rate = $poItem ? (float)$poItem->rate : (float)($mixDesign?->rate_per_qty ??
                                        0);
                                        $taxModel = $poItem?->tax; $taxRate = 0.0; $taxGroup = ''; $taxName = '-';
                                        $priceTax = 0.0;
                                        if ($taxModel) { $taxRate = (float)$taxModel->tax_rate; $taxGroup =
                                        $taxModel->tax_group ?? ''; $taxName = $taxModel->tax_name ?? ''; }
                                        $subtotal = $qty * $rate;
                                        if ($isTaxInclusive) { $total = $subtotal; $untaxedAmt = $total / (1 + ($taxRate
                                        / 100)); $priceTax = $total - $untaxedAmt; $unitPrice = $qty > 0 ? ($untaxedAmt
                                        / $qty) : $rate; }
                                        else { $untaxedAmt = $subtotal; $priceTax = $untaxedAmt * ($taxRate / 100);
                                        $total = $untaxedAmt + $priceTax; $unitPrice = $rate; }
                                        if ($taxRate <= 0 && $poItem) { $itemTaxAmount=(float)$poItem->tax_amount;
                                            $itemUntaxedAmount = (float)($poItem->untaxed_amount ?? ($poItem->quantity *
                                            $poItem->rate));
                                            if ($itemTaxAmount > 0 && $itemUntaxedAmount > 0) {
                                            $taxRate = round(($itemTaxAmount / $itemUntaxedAmount) * 100, 2); $taxGroup
                                            = $isIntra ? 'GST' : 'IGST';
                                            $taxName = $taxGroup . ' ' . ($taxRate == floor($taxRate) ? (int)$taxRate :
                                            $taxRate) . '%';
                                            if ($isTaxInclusive) { $untaxedAmt = $total / (1 + ($taxRate / 100));
                                            $priceTax = $total - $untaxedAmt; $unitPrice = $qty > 0 ? ($untaxedAmt /
                                            $qty) : $rate; }
                                            else { $priceTax = $untaxedAmt * ($taxRate / 100); $total = $untaxedAmt +
                                            $priceTax; }
                                            }
                                            }
                                            $description = self::formatMixDesignDescription('', $mixDesign);
                                            $data['items'] = $mixDesign ? [[
                                            'no' => 1, 'name' => $mixDesign->design_name ?? 'Concrete Mix',
                                            'description' => $description,
                                            'hsn' => $mixDesign->hsn_code ?? '-', 'qty' => $qty, 'received_qty'=>
                                            (float)($salesOrder->produced_qty ?? 0),
                                            'unit' => $mixDesign->unit?->unit_code ?? 'm³', 'unit_price' => $unitPrice,
                                            'tax_name' => $taxName ?: '-',
                                            'tax_rate' => $taxRate, 'tax_group' => $taxGroup, 'tax_amount' => $priceTax,
                                            'total' => $total,
                                            ]] : [];
                                            }
                                            ...
                                            $showPumpRate = (!isset($data['settings']['pdf']['pump_rates']) ||
                                            $data['settings']['pdf']['pump_rates']) && $pumpRate > 0;
                                            ...
                                            $computedTaxLines = collect($taxLines)->map(fn($amt, $lbl) => ['label' =>
                                            $lbl, 'amount' => $amt])->values()->toArray();
                                            $data['totals'] = [
                                            ...
                                            'terms_text' => self::resolveTermsCondition($data['settings'], 'Sales
                                            Order', $salesOrder->plant_id, ''),
                                            'total_words' => self::numberToWords($data['totals']['grand_total'], 'INR'),
                                            'po_number' => '', 'project_name' => $salesOrder->site?->name ?? '',
                                            ];
                                            return $data;
                                            }
                                            ...
                                            $data = self::base();
                                            $data['settings'] = self::getCustomSettings($batch->salesOrder->plant_id,
                                            'delivery_challans');
                                            $data['doc_title'] = $data['settings']['pdf']['labels']['invoice_title'] ??
                                            'DELIVERY CHALLAN';
                                            $data['doc_no'] = 'B' . ($batch->batch_no ?? $batch->id);
                                            $data['doc_date'] = optional($batch->load_time ??
                                            $batch->created_at)->format('d/m/Y H:i');
                                            $dispatch = $batch->dispatches->first();
                                            $data['delivery_date'] = $dispatch?->load_time ?
                                            \Carbon\Carbon::parse($dispatch->load_time)->format('d/m/Y H:i') :
                                            ($batch->load_time ? $batch->load_time->format('d/m/Y H:i') : 'N/A');
                                            ...
                                            $data['ship_to'] = self::formatShipTo($batch->salesOrder->site,
                                            $data['bill_to']);

                                            $settings =
                                            \App\Models\CustomSetting::getForModule($batch->salesOrder->plant_id,
                                            'batching');
                                            $printMode = $settings['material_print_mode'] ?? 'run';
                                            $formattedMaterials = $batch->getFormattedMaterials($printMode);

                                            $groupedMaterials = $formattedMaterials->groupBy(function($mat){ return
                                            $mat->material_name; })->map(function($group){
                                            $first = $group->first(); $target = $group->sum('target_qty'); $actual =
                                            $group->sum('actual_qty');
                                            return
                                            (object)['material_name'=>$first->material_name,'uom_code'=>$first->uom_code,'hsn_code'=>'-','target_qty'=>$target,'actual_qty'=>$actual,'deviation_quantity'=>$group->sum('deviation_quantity')];
                                            });
                                            $itemsList = [];
                                            if ($dispatch) {
                                            $mixDesign = $batch->salesOrder->mixDesign; $mixDesignName =
                                            $mixDesign?->design_name ?? ($mixDesign?->concrete_grade?->name ?? 'Concrete
                                            Mix');
                                            $qty = (float)($dispatch->delivered_qty ?: $batch->batch_size); $rate =
                                            (float)($dispatch->load_rate ?? 0);
                                            $subTotal = (float)($dispatch->load_untax_amount ?? ($qty * $rate));
                                            $taxAmount = (float)($dispatch->load_tax_amount ?? 0);
                                            $totalAmount = (float)($dispatch->load_total_amount ?? ($subTotal +
                                            $taxAmount)); $taxRate = $dispatch->loadTax?->rate ?? 0; $taxName =
                                            $dispatch->loadTax?->name ?? '-';
                                            $itemsList[] = ['no'=>1,'name'=>$mixDesignName,'description'=>"Concrete Mix
                                            Design - " . ($mixDesign?->design_code ??
                                            ''),'hsn'=>'3824','qty'=>$qty,'received_qty'=>$qty,'unit'=>$dispatch->uom?->unit_code
                                            ??
                                            'CBM','unit_price'=>$rate,'tax_name'=>$taxName,'tax_rate'=>$taxRate,'tax_amount'=>$taxAmount,'total'=>$totalAmount];
                                            }
                                            $sno = count($itemsList) + 1;
                                            foreach ($groupedMaterials->values() as $item) {
                                            $target = (float) $item->target_qty; $actual = (float) $item->actual_qty;
                                            $deviationVal = $actual - $target; $devPercent = 0;
                                            if ($target > 0) $devPercent = ($deviationVal / $target) * 100;
                                            $devSign = $devPercent > 0 ? '+' : '';
                                            $desc = sprintf("Target: %s %s | Actual: %s %s | Dev: %s%s%%",
                                            number_format($target, 2), $item->uom_code, number_format($actual, 2),
                                            $item->uom_code, $devSign, number_format($devPercent, 2));
                                            $itemsList[] =
                                            ['no'=>$sno++,'name'=>$item->material_name,'description'=>$desc,'hsn'=>$item->hsn_code,'qty'=>$actual
                                            ?:
                                            $target,'received_qty'=>$actual,'unit'=>$item->uom_code,'unit_price'=>0.00,'tax_name'=>'-','tax_rate'=>0.00,'tax_amount'=>0.00,'total'=>0.00];
                                            }
                                            $data['items'] = $itemsList;
                                            $taxLines = [];
                                            if ($dispatch && $dispatch->load_tax_amount > 0) $taxLines[] =
                                            ['name'=>$dispatch->loadTax?->name ??
                                            'GST','rate'=>$dispatch->loadTax?->rate ??
                                            18,'amount'=>(float)$dispatch->load_tax_amount];
                                            $data['totals'] = ['sub_total'=>(float)($dispatch?->load_untax_amount ??
                                            0),'discount'=>(float)($dispatch?->discount_amount ??
                                            0),'tax_lines'=>$taxLines,'shipping'=>(float)($dispatch?->transport_expenses
                                            ?? 0),'adjustment'=>(float)($dispatch?->adjustment_amount ??
                                            0),'round_off'=>(float)($dispatch?->round_off ??
                                            0),'grand_total'=>(float)($dispatch?->load_total_amount ?? 0)];
                                            $emptyWeight = (float) ($dispatch?->empty_weight_truck ?? 0); $loadedWeight
                                            = (float) ($dispatch?->loaded_weight_truck ?? 0);
                                            $netWeight = (float) ($dispatch?->net_weight ?? ($loadedWeight -
                                            $emptyWeight));
                                            ...
                                            (trim(($dispatch?->driver?->first_name ?? '') . ' ' .
                                            ($dispatch?->driver?->last_name ?? '')) ?: '-') . "\n" . "Empty Weight: " .
                                            $emptyWeightStr . " (" . ($dispatch?->empty_time ?
                                            \Carbon\Carbon::parse($dispatch->empty_time)->format('d-m-Y H:i') : '-') .
                                            ")\n" . "Loaded Weight: " . $loadedWeightStr . " (" . ($dispatch?->load_time
                                            ? \Carbon\Carbon::parse($dispatch->load_time)->format('d-m-Y H:i') : '-') .
                                            ")\n" . "Net Weight: " . $netWeightStr . "\n\n" . "Batch size: " .
                                            number_format((float) $batch->batch_size, 2) . " m³\n" . "Concrete Grade: "
                                            . ($batch->salesOrder?->mixDesign?->concrete_grade?->name ??
                                            ($batch->salesOrder?->mixDesign?->design_name ?? '-')) . " / Recipe Code: "
                                            . ($batch->salesOrder?->mixDesign?->design_code ?? '-');
                                            $data['meta'] = [
                                            'currency_code'=>'INR','currency_symbol'=>'₹','notes'=>$weightNotes,
                                            'terms_text'=>self::resolveTermsCondition($data['settings'], 'Delivery
                                            Challan', $batch->salesOrder->plant_id,
                                            $batch->salesOrder?->terms_conditions ?? "1. Goods received in good
                                            condition.\n2. Any variation in quantity to be reported immediately."),
                                            'total_words'=>'','po_number'=>$batch->salesOrder?->order_no ??
                                            '-','project_name'=>'Concrete Grade: ' .
                                            ($batch->salesOrder?->mixDesign?->concrete_grade?->name ?? '-'),
                                            ];
                                            $data['batch'] = $batch;
                                            return $data;
                                            ...
                                            {
                                            if ($num === 0) return '';
                                            if ($num < 20) return self::$ones[$num]; if ($num < 100) {
                                                $ten=(int)(floor($num / 10) * 10); $unit=$num % 10; return
                                                trim(self::$tens[$ten] . ($unit ? ' ' . self::$ones[$unit] : '' )); } if
                                                ($num < 1000) { $h=intdiv($num, 100); $rem=$num % 100; return
                                                trim(self::$ones[$h] . ' Hundred' . ($rem ? ' ' .
                                                self::convertBelowCrore($rem) : '' )); } if ($num < 100000) { //
                                                Thousand : 1,000 - 99,999 $th=intdiv($num, 1000); $rem=$num % 1000;
                                                return trim(self::convertBelowCrore($th) . ' Thousand' . ($rem ? ' ' .
                                                self::convertBelowCrore($rem) : '' )); } // Lakh : 1,00,000 - 99,99,999
                                                $lakh=intdiv($num, 100000); $rem=$num % 100000; return
                                                trim(self::convertBelowCrore($lakh) . ' Lakh' . ($rem ? ' ' .
                                                self::convertBelowCrore($rem) : '' )); } // Main Indian converter - max
                                                unit is Crore, bigger=Thousand/Lakh Crore private static function
                                                convertIndianTillCrore(int $num): string { if ($num===0) return '' ; if
                                                ($num < 10000000) { return self::convertBelowCrore($num); }
                                                $croreCount=intdiv($num, 10000000); $rem=$num % 10000000; // croreCount
                                                itself may be> 1 Lakh, convert it without Crore word to get
                                                Thousand/Lakh Crore
                                                $croreWords = self::convertBelowCrore($croreCount);
                                                // If croreCount >= 1 Crore (i.e. 100 Lakh), convertBelowCrore will give
                                                "One Hundred Lakh"
                                                // To avoid "Lakh Lakh" infinite, handle very large croreCount
                                                recursively via same logic
                                                ...
                                                {
                                                $number = (float) $number;
                                                $isNegative = $number < 0; $number=abs($number); $no=(int)
                                                    floor($number); $point=(int) round(($number - $no) * 100); if
                                                    ($point===100) { $no +=1; $point=0; } $integerWords=$no===0 ? 'Zero'
                                                    : self::convertIndianTillCrore($no);
                                                    $currencyLabel=$currency==='INR' ? 'Rupees' : $currency;
                                                    $result=$currencyLabel . ' ' . $integerWords; if ($point> 0) {
                                                    ...
                                                    public static function resolveTemplateKey(string $moduleKey, int
                                                    $plantId): string
                                                    {
                                                    $setting = PrintTemplateSetting::where('module_key',
                                                    $moduleKey)->where('plant_id', $plantId)->with('template')->first();
                                                    if (!$setting && $moduleKey === 'customer_pos') {
                                                    $setting = PrintTemplateSetting::where('module_key',
                                                    'quotations')->where('plant_id',
                                                    $plantId)->with('template')->first();
                                                    }
                                                    ...
                                                    {
                                                    return
                                                    ['standard','box_layout','elite','modern','spreadsheet','tallysheet','compact','indian_gst','formal_gst','standard_indigo','minimalist_lite','delivery_challan_a4'];
                                                    }

                                                    public static function resolveView(string $templateKey): string
                                                    {
                                                    if ($templateKey === 'delivery_challan_a4') return
                                                    "pdfs.batches.delivery_token";
                                                    $map = ['formal_gst' => 'indian_gst','standard_indigo' =>
                                                    'elite','minimalist_lite' => 'compact'];
                                                    $supported = self::supportedTemplates();
                                                    $key = in_array($templateKey, $supported) ? ($map[$templateKey] ??
                                                    $templateKey) : 'standard';
                                                    return "pdfs.templates.{$key}";
                                                    }

                                                    public static function getCustomSettings(int $plantId, string
                                                    $module, ?string $templateKey = null): array
                                                    {
                                                    $defaults = self::getDefaultSettings($module);
                                                    $baseStored = \App\Models\CustomSetting::getForModule($plantId,
                                                    $module);
                                                    ...
                                                    switch ($module) {
                                                    case 'invoices': case 'gst_invoices': $invoiceTitle = 'TAX INVOICE';
                                                    break;
                                                    ...
                                                    'upi_qr'=>true,
                                                    'pump_rates'=>true,
                                                    'labels' => ['invoice_title'=>$invoiceTitle,'bill_to'=>'Bill
                                                    To','ship_to'=>'Ship To','rate'=>'Rate','amount'=>'Amount']
                                                    ],
