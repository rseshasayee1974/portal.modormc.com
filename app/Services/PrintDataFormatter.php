            ...
            $mixDesign = $item->mixDesign 
                ?: ($dispatch?->mixDesign 
                    ?: ($salesOrder?->mixDesign 
                        ?: ($item->item_id ? \App\Models\MixDesign::with(['concreteGrade', 'concrete_grade', 'items.product', 'items.uom'])->find($item->item_id) : null)
                    )
                );
            $gradeName = self::resolveConcreteGrade($mixDesign, $item->item_name);
