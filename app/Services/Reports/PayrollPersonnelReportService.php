    ...
    public function generate(array $params): array
    {
            ...
            if ($matchingPayslips->isNotEmpty()) {
                foreach ($matchingPayslips as $slip) {
                    $rows->push([
                        ...
                        'dept_designation'           => $deptName . ($desigName ? ' / ' . $desigName : ''),
                        ...
                        'attendance_summary'         => 'P:' . (float)$slip->present_days . ((float)$slip->paid_leave_days > 0 ? ' L:' . (float)$slip->paid_leave_days : '') . ((float)$slip->absent_days > 0 ? ' A:' . (float)$slip->absent_days : ''),
                        ...
                        'total_earnings_formatted'   => '₹ ' . number_format((float)$slip->total_earnings, 2),
                        'total_deductions_formatted' => (float)$slip->total_deductions > 0 ? '- ₹ ' . number_format((float)$slip->total_deductions, 2) : '₹ 0.00',
                        'net_salary_formatted'       => '₹ ' . number_format((float)$slip->net_salary, 2),
        ...
        return [
            ...
            'summary' => [
                ...
                'total_working_days'     => (float)$rows->sum('working_days'),
                'total_present_days'     => (float)$rows->sum('present_days'),
                'total_paid_leave_days'  => (float)$rows->sum('paid_leave_days'),
                'total_absent_days'      => (float)$rows->sum('absent_days'),
